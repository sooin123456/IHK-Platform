    ...
    public class ExportElementPropertiesCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                {
                    Title = "Lukas QTO - 요소 Properties CSV 위치",
                ...
                TaskDialog.Show("Lukas QTO", "요소 Properties CSV 생성 완료:\n" + path + "\n" + evidencePath + "\n\n범위: " + scope + "\n요소: " + rows.Count + "\n링크 제외: " + counts.ExcludedLinkInstanceCount + "\n체적·길이·높이의 ZERO와 MISSING은 구분되어 기록됩니다.");
        ...
        {
            var dialog = new TaskDialog("Lukas QTO - Properties 추출 범위") { MainInstruction = "Revit Properties 추출 범위를 선택하십시오.", MainContent = "링크 모델은 어떤 범위에서도 제외됩니다. 선택 범위는 현재 선택한 host 요소만, 활성 뷰 범위는 현재 뷰의 host 요소만, 전체는 현재 host 모델만 읽습니다." };
        ...
        {
            string json = "{\n" +
                "  \"schema_version\": \"revit-properties-evidence-v1\",\n" +
