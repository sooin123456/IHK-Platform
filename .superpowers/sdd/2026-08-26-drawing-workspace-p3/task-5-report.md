...
- Before canonical de-duplication, the shared reader compares the complete raw operation-ID multisets from `operations` and `operationOrder`. One-sided duplicate contributions now fail identically in the reader, adapter, persisted-state validator, and live service; exactly paired duplicates remain idempotent.
...
- Final rereview RED: both `operations=[A,A]/order=[A]` and `operations=[A]/order=[A,A]` passed reader, adapter, and persisted validation despite live insertion rejection.
...
- `git diff --check`: pass.
- Final rereview GREEN: protocol, service, adapter, command, document-store, and outbox suite 188/188; real Chromium 3/3 with 100-operation recovery, two same-origin browser realms sharing IndexedDB, valid server-origin freeze-after-persist recovery, and no quarantine.
