# 한길시스템 BIM 적산 플랫폼 실제 제품 기획

기준일: 2026-08-13  
사용자: 설계·BIM·견적 실무자 및 한길시스템 고객
외부 제품명: **한길시스템** (`Lukas.Qto`·`Lukas.Qto`는 기존 DLL·경로·네임스페이스 호환용 내부 식별자)

## 0-A. 2026-08-14 사업·배포 결정

- 하나의 외부 브랜드만 사용한다: **BIM 건축 적산·물량산출 전문 | 한길시스템**.
- 서비스의 첫 판매 단위는 **BIM·물량 근거 패키지**다.
- 고객 흐름은 `문의 접수 → 견적 검토 → 작업 확정 → BIM 모델링 → 물량산출 → 전문가 검토 → 납품 완료`다.
- 내역서·공사비는 한길시스템 전문가 서비스로 제공하되 플랫폼 자동화는 `준비 중`으로 표시한다.
- 웹은 소개 페이지에 그치지 않고 Supabase의 비공개 프로젝트·원본 SHA·검토 기록을 고객에게 제공한다.
- 첫 공식 베타 설치본은 **Revit 2025 전용 사전 빌드 ZIP**이다. 테스터에게 SDK·소스 빌드를 요구하지 않는다.
- 리본의 기존 기능은 모두 유지한다. 대표 기능은 `IFC·QTO 내보내기`, 독립 검증 기능은 `Properties 추출`이다.
- 내부 AddInId, 진입 클래스 `Lukas.Qto.App`, DLL 파일명은 설치 호환성을 위해 변경하지 않는다.

## 0-B. 증거 기반 적산 데이터 플랫폼 목표

Lukas QTO의 장기 제품 목표는 “AI가 적산 숫자를 대신 확정하는 서비스”가 아니라
**어떤 원본·규칙·수정·승인으로 수량과 금액이 확정됐는지 증명하는 적산 데이터
플랫폼**이다. 구현 순서는 정확한 Revit 추출을 앞에 두고 다음 계층을 순차 연결한다.

1. Element ID·IFC Global ID·파일 SHA·Revit 버전·추출 범위를 변경 불가능한 근거로 보존한다.
2. 기본수량·보정수량·최종수량과 적용 규칙·규칙 버전·승인자를 서로 다른 필드와 이력으로 보존한다.
3. 자동화는 공종 분류·내역 매핑·누락·중복·이상치·개정 차이를 **제안**할 수 있다.
4. 제안은 원본 SHA·생성기 종류(`rule` 또는 `ai`)·생성기 버전·근거와 함께 저장하며 원본 수량을 수정하지 않는다.
5. 사람의 승인·기각·보류는 제안과 분리된 append-only 이력으로 남긴다. 최종 물량·금액은 승인된 결정론적 계산식만 확정한다.
6. 향후 계약·기성·정산 결과는 새 원본 근거로 연결해 예측·추천용 학습 데이터로 축적하되, 과거 승인 이력을 덮어쓰지 않는다.

첫 수직 기능은 `ELEMENT_LEDGER_REVIEW_V2`이다. 요소 원장의 중복 ID, 수량 상태
오류, 분류·속성 누락을 결정론적 규칙으로 제안하고, 동일 Element ID의 개정 전후
체적·길이·높이 차이를 lossless decimal로 계산해 사람이 결정을 기록한다. 값이 없는
차원은 0으로 만들지 않고 미산정 건수로 분리한다. 외부
LLM 연동은 이 계약과 실제 검토 데이터가 쌓인 뒤 별도 평가셋으로 정확도·비용·보안
기준을 통과한 경우에만 추가한다.

## 0. 제품 우선순위 결정

현장 사용자 피드백에 따라 제품의 본체를 **근거 검산기**가 아니라
...
한길시스템 플랫폼은 Revit 애드인, Windows 계산 엔진, 비공개 웹 프로젝트에서 세 작업을 이어 준다.
...
- Revit 내부: `한길시스템` 리본의 **IFC·QTO 내보내기**
- Revit 밖: 설치형 계산·검산 엔진(운영 단계에서는 한길시스템 담당자가 사용)
- 고객 전달: Supabase 비공개 프로젝트와 원본 파일 SHA, 필요 시 로컬 결과 ZIP
- 계산: 기존 `Lukas.Qto.Core`를 양쪽에서 재사용

웹은 회사 소개·문의·고객 프로젝트·파일 전달을 맡고, RVT 직접 처리와 결정론적 계산은 Revit/Windows가 맡는다.

## 2. 이번 범위에서 하지 않는 것

Ponytail 원칙에 따라 실제 사용 증거가 생기기 전에는 다음을 만들지 않는다.

- 결제 자동화, 실시간 공동편집, 웹에서 RVT 직접 실행
- 모델 뷰어와 자체 IFC 파서
- AI가 승인 없이 매핑·수량·금액·PASS/FAIL을 확정하는 기능
...
| 모듈 | 책임 | 금지 |
|---|---|---|
| `Lukas.Qto` Revit Add-in | 활성 문서 접근, IFC Export API 호출, QTO 추출, 파일 저장, Revit UI | 내역 검산·금액 판정 |
| `Lukas.Qto.Core` | CSV/XLSX 입력, 소스 게이트, 계산 규칙, 보고서·manifest 생성 | Revit API·화면·네트워크 |
| `Lukas.Qto.Preflight` | 자동화와 장애 진단을 위한 기존 CLI | 별도 계산식 복제 |
