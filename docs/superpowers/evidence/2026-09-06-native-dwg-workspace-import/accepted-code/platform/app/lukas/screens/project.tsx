...
import {
  clearPendingProjectUpload,
  completePendingProjectUpload,
  finalizePendingProjectUpload,
  loadPendingProjectUpload,
  projectUploadFinalizationPath,
  projectUploadContentType,
  savePendingProjectUpload,
  uploadProjectFileResumable,
  type PendingProjectUpload,
  type ProjectFileUploadMetadata,
} from "~/lukas/lib/project-file-upload";
import {
...
async function projectUploadBrowserSession() {
  const url = import.meta.env.VITE_SUPABASE_URL;
  const key = import.meta.env.VITE_SUPABASE_ANON_KEY;
  if (!url || !key)
    throw new Error("파일 저장소 연결 설정을 확인하지 못했습니다.");
  ...
  if (error || !session?.access_token)
    throw new Error("로그인 연결을 확인하지 못했습니다. 다시 로그인해주세요.");
  return {
...
async function verifyProjectUpload(
  client: SupabaseClient<Database>,
  accessToken: string,
  metadata: ProjectFileUploadMetadata,
) {
  const { data: verification, error } = await client.functions.invoke(
    "lukas-qto-upload-verify",
    {
      body: metadata,
  ...
  if (error) {
    ...
    if (context) {
      try {
        ...
        // The SDK message remains useful when the response is not JSON.
      }
    }
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  try {
    ...
    if (
      ...
      throw new Response("프로젝트 자료를 불러오지 못했습니다.", {
        status: 500,
      });
...
type AuthenticatedProjectActionArgs = Pick<
  Route.ActionArgs,
  "params" | "request"
> & {
  client: SupabaseClient<Database>;
...
type AuthenticatedProjectActionContext = Pick<
  AuthenticatedProjectActionArgs,
  "client" | "headers" | "user"
>;

export async function action(
  { request, params }: Route.ActionArgs,
  authenticatedContext?: AuthenticatedProjectActionContext,
) {
  let authenticated = authenticatedContext;
  if (!authenticated) {
    const [client, headers] = makeServerClient(request);
    const {
      data: { user },
    ...
    if (!user || user.is_anonymous)
      throw redirect(authLoginPath(request.url), { headers });
    authenticated = { client, headers, user };
  }

  return projectActionForAuthenticatedUser({
    ...authenticated,
    params,
    request,
  });
...
async function projectActionForAuthenticatedUser({
  client,
  headers,
  params,
  request,
  ...
  let uploadReturnTarget: ReturnType<
    typeof parseDrawingWorkspaceUploadReturnTarget
  >;
  try {
    const rawReturnTo = formData.get("return_to");
    if (rawReturnTo !== null && typeof rawReturnTo !== "string") throw null;
    uploadReturnTarget = parseDrawingWorkspaceUploadReturnTarget(
      project.id,
      rawReturnTo,
    );
  } catch {
    return data(
      { error: "작업실 복귀 주소가 올바르지 않습니다." },
      { status: 400, headers },
    );
  }
  ...
  const { data: rawVerifiedUpload, error: verifiedUploadError } =
    await adminClient
      ...
      .select(
        "id, actor_id, project_id, kind, storage_path, original_filename, content_type, byte_size, sha256, expires_at, consumed_file_id, dwg_header_version",
      )
  ...
  if (!fileMatchesProjectKind(validatedKind, originalFilename)) {
    return data(
      {
        error:
          projectFileKindMismatchMessage(validatedKind, originalFilename) ??
          `${kindLabel(validatedKind)}에 맞는 파일을 선택하세요. ${projectFileKindPolicy[validatedKind].help}`,
      },
      { status: 400, headers },
    );
  }
  if (!contentTypeMatchesProjectKind(validatedKind, contentType))
    return data(
      {
        error: `${validatedKind === "dwg" ? "DWG" : "DXF"} 파일 형식 정보가 허용되지 않습니다.`,
      },
      { status: 400, headers },
    );
  if (uploadReturnTarget) {
    if (
      validatedKind !== "dxf" &&
      validatedKind !== "dwg" &&
      validatedKind !== "pdf"
      ...
      return data(
        {
          error:
            "작업실 복귀는 PDF, DXF 또는 DWG 업로드에서만 사용할 수 있습니다.",
        },
        { status: 400, headers },
      );
    let capability;
    try {
      capability = await loadDrawingWorkspaceCapability(
        client as unknown as SupabaseClient<DrawingWorkspaceDatabase>,
        project.id,
        user.id,
        project.owner_id,
        user.app_metadata.role === "hangil_staff" ? "staff" : null,
      );
    } catch {
      return data(
        { error: "작업실 편집 권한을 확인하지 못했습니다." },
        { status: 500, headers },
      );
    }
    if (capability !== "admin" && capability !== "editor")
      return data(
        { error: "이 작업실을 편집할 권한이 없습니다." },
        { status: 403, headers },
      );
    const drawingClient =
      client as unknown as SupabaseClient<DrawingWorkspaceDatabase>;
    const { data: targetRevision, error: targetRevisionError } =
      await drawingClient
        .from("lukas_drawing_revisions")
        .select("id,document_id,project_id,status")
        .eq("project_id", project.id)
        .eq("document_id", uploadReturnTarget.workspaceId)
        .order("sequence", { ascending: false })
        .limit(1)
        .maybeSingle();
    if (targetRevisionError)
      return data(
        { error: "복귀할 작업실을 확인하지 못했습니다." },
        { status: 500, headers },
      );
    if (
      !targetRevision ||
      targetRevision.project_id !== project.id ||
      targetRevision.document_id !== uploadReturnTarget.workspaceId
    )
      return data(
        {
          error:
            "복귀할 작업실을 찾을 수 없습니다. 작업실에서 다시 업로드해 주세요.",
        },
        { status: 409, headers },
      );
    if (targetRevision.status !== "draft")
      return data(
        { error: "초안 작업실에서만 PDF, DXF 또는 DWG를 올릴 수 있습니다." },
        { status: 409, headers },
      );
  }
  if (
  ...
  const destination = projectUploadDestination({
    fileId: finalizedUpload.fileId,
    kind: validatedKind,
    projectId: project.id,
    returnPath,
    returnTo: uploadReturnTarget?.path,
  });
  if (formData.get("upload_response_mode") === "browser_recovery")
    return data({ destination }, { headers });
...
export default function Project({
  ...
  const requestedUploadReturnTo = searchParams.get("returnTo");
  const [selectedUploadKind, setUploadKind] = useState<ProjectFileKind>(() =>
    projectFileKinds.includes(requestedUploadKind as ProjectFileKind)
      ? (requestedUploadKind as ProjectFileKind)
      : "ifc",
  );
  const uploadKind = selectedUploadKind;
  useEffect(() => {
  ...
  const [pendingProjectUpload, setPendingProjectUpload] =
    useState<PendingProjectUpload | null>(null);
  useEffect(() => {
  ...
  const completeBrowserUpload = useCallback(
    async (
      ...
      const actionUrl = new URL(
        projectUploadFinalizationPath(pending.projectId),
        window.location.origin,
      );
      const completed = await completePendingProjectUpload({
        finalize: (verified) =>
          finalizePendingProjectUpload({
        ...
        pending,
        persist: async (nextPending, transition) => {
          const saved = await savePendingProjectUpload(
            nextPending,
            undefined,
            undefined,
            transition,
          ...
          if (saved) {
            setPendingProjectUpload(saved);
          }

          return saved;
        },
        verify: (metadata) =>
          verifyProjectUpload(client, accessToken, metadata),
      });
      await clearPendingProjectUpload(
        completed.pending.actorId,
        completed.pending.projectId,
        completed.pending.uploadId,
        completed.pending.revision,
      );
      setPendingProjectUpload(null);
      navigate(completed.destination, { replace: true });
    },
    [navigate],
  );

  const recoverCompletedUpload = useCallback(
    async (pending: PendingProjectUpload) => {
      setDirectUploadBusy(true);
      setDirectUploadError(null);
      try {
        const { accessToken, actorId, client } =
          await projectUploadBrowserSession();
        if (actorId !== pending.actorId)
          throw new Error(
            "로그인 사용자가 달라 이전 업로드를 복구할 수 없습니다.",
          );
        await completeBrowserUpload(pending, client, accessToken);
      } catch (error) {
        setDirectUploadBusy(false);
        setDirectUploadError(
          `원본 저장은 완료됐지만 파일 등록을 마치지 못했습니다: ${
            error instanceof Error
              ? error.message
              : "업로드 상태를 확인하지 못했습니다."
          } 아래의 ‘파일 등록 다시 시도’를 누르면 원본을 다시 올리지 않고 이어서 처리합니다.`,
        );
      }
    },
  ...
  const recoveryAttemptedScope = useRef<string | null>(null);
  useEffect(() => {
    if (!pathname.endsWith("/files")) return;
    const scope = `${loaderData.currentUserId}:${loaderData.project.id}`;
    if (recoveryAttemptedScope.current === scope) return;
    recoveryAttemptedScope.current = scope;
    const pending = loadPendingProjectUpload(
      loaderData.currentUserId,
      loaderData.project.owner_id,
      loaderData.project.id,
    );
    if (!pending) return;
    setPendingProjectUpload(pending);
    if (pending.uploadComplete) {
      void recoverCompletedUpload(pending);
      return;
    }
    setDirectUploadError(
      `‘${pending.originalFilename}’ 업로드가 중단됐습니다. 같은 파일을 선택하면 저장된 지점부터 이어서 올립니다.`,
    );
  }, [
    loaderData.currentUserId,
    loaderData.project.id,
    loaderData.project.owner_id,
    pathname,
    recoverCompletedUpload,
  ]);

  async function discardPendingUpload() {
    if (!pendingProjectUpload) return;
    const cleared = await clearPendingProjectUpload(
      pendingProjectUpload.actorId,
      pendingProjectUpload.projectId,
      pendingProjectUpload.uploadId,
      pendingProjectUpload.revision,
    );
    if (!cleared) {
      setDirectUploadError(
        "다른 탭에서 업로드 상태가 바뀌었습니다. 페이지를 새로고침해 확인해주세요.",
      );
      return;
    }
    setPendingProjectUpload(null);
  ...
  async function handleSourceUpload(event: FormEvent<HTMLFormElement>) {
    ...
    setDirectUploadBusy(true);
    try {
      const { accessToken, actorId, client, supabaseUrl } =
        await projectUploadBrowserSession();
      const returnToValue = form.get("return_to");
      const returnTo =
        typeof returnToValue === "string" && returnToValue
          ? returnToValue
          : null;
      const savedPending = loadPendingProjectUpload(
        actorId,
        loaderData.project.owner_id,
        loaderData.project.id,
      );
      if (savedPending?.uploadComplete) {
        setPendingProjectUpload(savedPending);
        await recoverCompletedUpload(savedPending);
        return;
      }
      const contentType = projectUploadContentType(
        kind as ProjectFileKind,
        file.type,
      );
      if (
        savedPending &&
        (savedPending.kind !== kind ||
          savedPending.originalFilename !== file.name.trim() ||
          savedPending.byteSize !== file.size ||
          savedPending.contentType !== contentType ||
          savedPending.returnTo !== returnTo)
      )
        throw new Error(
          "다른 파일의 중단된 업로드가 있습니다. 기존 복구 기록을 지운 뒤 새 파일을 선택해주세요.",
        );
      const uploadId = savedPending?.uploadId ?? crypto.randomUUID();
      let completedPending: PendingProjectUpload | null = null;
      let journalRevision = savedPending?.revision ?? 0;
      ...
      const rememberStoragePath = async (
        metadata: ProjectFileUploadMetadata,
      ) => {
        const pending = pendingFromMetadata(metadata, false);
        const saved = await savePendingProjectUpload(pending);
        if (!saved)
          throw new Error(
            "업로드 복구 정보를 저장하지 못했습니다. 다른 탭을 닫고 브라우저 저장소를 확인한 뒤 다시 시도해주세요.",
          );
        journalRevision = saved.revision;
        setPendingProjectUpload(saved);
      };
      const rememberCompletedUpload = async (
        metadata: ProjectFileUploadMetadata,
      ) => {
        const pending = pendingFromMetadata(metadata, true);
        const saved = await savePendingProjectUpload(pending);
        if (!saved)
          throw new Error(
            "완료된 업로드 복구 정보를 저장하지 못했습니다. 다른 탭을 닫고 다시 시도해주세요.",
          );
        journalRevision = saved.revision;
        completedPending = saved;
        setPendingProjectUpload(saved);
      };
      await uploadProjectFileResumable({
        ...
        onUploadComplete: rememberCompletedUpload,
        onProgress: setDirectUploadProgress,
        onStoragePath: rememberStoragePath,
        ownerId: loaderData.project.owner_id,
        projectId: loaderData.project.id,
        resumeStoragePath: savedPending?.storagePath,
        supabaseUrl,
      });
      setDirectUploadProgress(100);
      if (!completedPending)
        throw new Error("완료된 업로드 복구 정보를 확인하지 못했습니다.");
      await completeBrowserUpload(completedPending, client, accessToken);
    } catch (error) {
      setDirectUploadBusy(false);
      setDirectUploadError(
        error instanceof Error
          ...
          {loaderData.canCreateWorkspace ? (
            ...
            >
              <section className="rounded-2xl border bg-card p-6 shadow-sm">
                ...
                {pendingProjectUpload?.uploadComplete ? (
                  <div className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
                    <p className="font-semibold">
                      원본 저장 완료 · 파일 등록 대기
                    </p>
                    <p className="mt-1 break-all text-xs text-amber-800">
                      {pendingProjectUpload.originalFilename}
                    </p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Button
                        disabled={uploadBusy}
                        onClick={() =>
                          void recoverCompletedUpload(pendingProjectUpload)
                        }
                        type="button"
                      >
                        {uploadBusy ? "파일 등록 중…" : "파일 등록 다시 시도"}
                      </Button>
                      <Button
                        disabled={uploadBusy}
                  ...
                  <>
                    {pendingProjectUpload ? (
                      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900">
                        <span>
                          중단된 ‘{pendingProjectUpload.originalFilename}’을
                          같은 파일로 이어서 올릴 수 있습니다.
                        </span>
                        <Button
                          disabled={uploadBusy}
                          onClick={discardPendingUpload}
                    ...
                    >
                      ...
                      {requestedUploadReturnTo ? (
                        <input
                          name="return_to"
                          type="hidden"
                          value={requestedUploadReturnTo}
                        />
                      ) : null}
              ...
              <aside className="rounded-2xl border bg-card p-6 shadow-sm">
                ...
                <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
                  <li>• IFC·PDF·DXF·DWG·CSV 원본은 수정하지 않습니다.</li>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex shrink-0 flex-col items-end gap-2 text-sm">
                          ...
                          {file.kind === "dwg" ? (
                            <Link
                              className="font-medium text-[#3024d8]"
                              to={`/projects/${loaderData.project.id}/workspaces/new?sourceFileId=${file.id}`}
                            >
                              작업실에서 열기
                            </Link>
                          ) : null}
