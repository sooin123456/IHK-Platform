...
import {
  ...
  DrawingMeasurementCalibrationRequiredError,
...
export type DrawingWorkspaceSourceAttachCandidate =
  DrawingWorkspaceSourceIdentity & {
    kind: "pdf";
    createdAt: string;
  };
...
type DrawingObjectSourceRow = {
  ...
  source_file_id: string;
  source_sha256: string;
  source_kind: "pdf_region" | "ifc_element" | "dxf_entity" | "dwg_entity";
...
export type DrawingWorkspaceDatabase = Omit<Database, "public"> & {
  public: Omit<Database["public"], "Tables" | "Functions"> & {
    ...
    Functions: Database["public"]["Functions"] & {
      lukas_qto_create_drawing_share: DrawingRpc<{
        p_actor_id: string;
        p_project_id: string;
        p_document_id: string;
        p_revision_id: string;
        p_revision_version: number;
        p_snapshot_sha256: string;
        p_token_hash: string;
        p_request_id: string;
      }>;
      lukas_qto_list_drawing_shares: DrawingRpc<{
        p_project_id: string;
        p_document_id: string;
        p_revision_id: string;
        p_revision_version: number;
      ...
      lukas_qto_revoke_drawing_share: DrawingRpc<{
        p_project_id: string;
        p_document_id: string;
        p_revision_id: string;
        p_revision_version: number;
      ...
      lukas_qto_shared_drawing_revision: DrawingRpc<{ p_token: string }>;
      ...
      lukas_drawing_attach_source: DrawingRpc<{
        p_document_id: string;
        p_revision_id: string;
        p_canvas_id: string;
      ...
      lukas_drawing_attest_dxf_import_plan: DrawingRpc<{
        p_actor_id: string;
        p_project_id: string;
        p_revision_id: string;
        p_canvas_id: string;
      ...
      lukas_drawing_discard_operation_suffix: DrawingRpc<{
        p_revision_id: string;
        p_operations: Json;
      }>;
      ...
      lukas_drawing_ifc_derivative_job_status: DrawingRpc<{
        p_source_file_id: string;
        p_source_sha256: string;
      }>;
    };
...
const MAX_DRAWING_CONFLICT_DISCARD_OPERATIONS = 256;
...
const allowedFormFields = {
  ...
  discard_conflicted_operations: new Set([
    "intent",
    "revision_id",
    "operations_json",
  ]),
...
export function parseWorkspaceMutation(form: FormData): WorkspaceMutation {
  ...
  if (knownIntent === "discard_conflicted_operations") {
    const revisionId = Uuid.parse(form.get("revision_id"));
    const value = jsonFormValue(form, "operations_json");
    if (
      !Array.isArray(value) ||
      value.length < 1 ||
      value.length > MAX_DRAWING_CONFLICT_DISCARD_OPERATIONS
    )
      throw workspaceMutationValidationError(
        "operations_json",
        "폐기할 충돌 작업 범위가 올바르지 않습니다.",
      );
    const operations = value.map(parseOperation);
    if (operations.some((operation) => operation.revisionId !== revisionId))
      throw workspaceMutationValidationError(
        "operations_json",
        "폐기할 작업의 도면 리비전이 일치하지 않습니다.",
      );
    return {
      intent: "discard_conflicted_operations",
      revisionId,
...
const CollaborationCanonicalCanvasSchema = z
  .unknown()
  .refine(
    (value) => DrawingCanvasSchema.safeParse(value).success,
    "Drawing canonical canvas dimensions are invalid.",
  );
...
const CollaborationCanonicalJsonSchema = z
  .object({
    ...
    canvases: z.array(CollaborationCanonicalCanvasSchema),
...
const CollaborationCanonicalIssueSchema = z
  .object({ id: Uuid, objectId: Uuid })
  .strict();
        ...
        drawingCollaborationOperationDigestSource(
          {
            ...
            baseVersions: value.baseVersions,
            forward: value.forward,
            inverse: value.inverse,
            ...(value.originalOperationId && value.historyAction
              ? {
...
const DrawingWorkspaceCollaborationBootstrapSchema = z
  ...
  .superRefine((value, context) => {
    ...
    if (
      new Set(value.recentOutcomes.map((outcome) => outcome.clientOperationId))
        .size !== value.recentOutcomes.length ||
      new Set(value.recentOutcomes.map((outcome) => outcome.sequence)).size !==
        value.recentOutcomes.length ||
      value.recentOutcomes.some(
        (outcome, index) =>
          outcome.revisionId !== value.canonicalJson.revision.id ||
          outcome.sequence > value.operationSequence ||
          (index > 0 &&
            value.recentOutcomes[index - 1].sequence >= outcome.sequence),
      )
    )
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["recentOutcomes"],
        message: "Drawing collaboration bootstrap outcomes are invalid.",
      });
  });
...
const AuthorizedMeasurementLayerSchema = z
  .object({
    id: Uuid,
    pageId: Uuid,
    canvasId: Uuid,
  ...
  .strict()
  .transform(({ pageId: _pageId, ...layer }) =>
    DrawingStructureLayerSchema.parse(layer),
  );
...
export function deriveAuthorizedDrawingMeasurementEvidence(
  ...
  const canvases = bootstrap.canonicalJson.canvases.map((input) =>
    DrawingCanvasSchema.parse(input),
  );
  const canvasMap = Object.fromEntries(
    canvases.map((canvas) => [canvas.id, canvas]),
  );
  if (Object.keys(canvasMap).length !== canvases.length)
    throw new Error("Drawing measurement canvas IDs are inconsistent.");
  const layers = bootstrap.canonicalJson.layers.map((input) =>
    AuthorizedMeasurementLayerSchema.parse(input),
  );
  const layerMap = Object.fromEntries(layers.map((layer) => [layer.id, layer]));
  if (Object.keys(layerMap).length !== layers.length)
    throw new Error("Drawing measurement layer IDs are inconsistent.");
...
export function deriveAuthorizedDrawingMeasurementEvidenceResult(
  ...
  try {
    ...
    if (
      error instanceof DrawingMeasurementCalibrationRequiredError ||
...
export function assertDrawingWorkspaceCollaborationBootstrapLineage(
  bootstrap: DrawingWorkspaceCollaborationBootstrap,
  expected: Pick<
  ...
  if (bootstrap.canonicalJson.revision.id !== expected.revisionId)
    throw new DrawingWorkspaceRpcError(
      "Drawing measurement revision identity is inconsistent.",
    );
  if (
    bootstrap.canonicalJson.revision.documentId !== expected.documentId ||
    (expected.projectId !== undefined &&
      bootstrap.canonicalJson.revision.projectId !== expected.projectId)
  )
    throw new DrawingWorkspaceRpcError(
      "Drawing measurement document lineage is inconsistent.",
    );
  if (bootstrap.canonicalJson.revision.version !== expected.revisionVersion)
    throw new DrawingWorkspaceRpcError(
...
export async function loadDrawingWorkspaceMeasurementState(
  ...
  assertDrawingWorkspaceCollaborationBootstrapLineage(
...
const P2CanvasRowSchema = z
  .object({
    ...
    width_mm: z.number(),
    height_mm: z.number(),
    output_profile: z.unknown().nullable().optional(),
...
const P2SourceRowSchema = z
  .object({
    ...
    dxf_unit_code: z.number().int().nullable().default(null),
    dxf_unit_source: z.string().nullable().default(null),
    dxf_importer_version: z.number().int().nullable().default(null),
    version: z.number().int().positive(),
  ...
  .superRefine((source, context) => {
    const dxfFields = [
      source.dxf_entity_key,
      source.dxf_entity_type,
      source.dxf_source_layer,
      source.dxf_handle,
      source.dxf_unit_code,
      source.dxf_unit_source,
      source.dxf_importer_version,
    ];
    const pdfFields = [
    ...
    const ifcFields = [source.element_id, source.ifc_global_id, source.camera];
    if (
      source.source_kind === "pdf_region"
        ? dxfFields.some((value) => value !== null) ||
          ifcFields.some((value) => value !== null) ||
          source.dwg_entity_json !== null
        : source.source_kind === "ifc_element"
          ? dxfFields.some((value) => value !== null) ||
            pdfFields.some((value) => value !== null) ||
            source.dwg_entity_json !== null
          : source.source_kind === "dxf_entity"
            ? pdfFields.some((value) => value !== null) ||
              ifcFields.some((value) => value !== null) ||
              source.dwg_entity_json !== null ||
              [
      ...
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Drawing source payload does not match its kind.",
      });
  });
...
export async function loadDrawingRevisionSources(
  ...
  const rows = await loadAllDrawingRows<DrawingObjectSourceRow>(client, {
    ...
    select:
      "id,object_id,revision_id,project_id,source_file_id,source_sha256,source_kind,pdf_page_number,x,y,width,height,element_id,ifc_global_id,camera:camera_json,dwg_entity_json,dxf_entity_key,dxf_entity_type,dxf_source_layer,dxf_handle,dxf_unit_code,dxf_unit_source,dxf_importer_version,version,status",
  });
  return rows.map((row) => {
    ...
    const nativePayload =
      value.source_kind === "dwg_entity"
        ? DrawingNativeDwgEntityPayloadSchema.safeParse(value.dwg_entity_json)
        : null;
    if (nativePayload && !nativePayload.success)
      throw new Error("Drawing source metadata is invalid.");
        ...
        : value.source_kind === "ifc_element"
          ...
          : value.source_kind === "dxf_entity"
            ? {
                ...
                sourceSha256: value.source_sha256,
                sourceKind: value.source_kind,
                entityKey: value.dxf_entity_key,
                entityType: value.dxf_entity_type,
                sourceLayer: value.dxf_source_layer,
                handle: value.dxf_handle,
                unitCode: value.dxf_unit_code,
                unitSource: value.dxf_unit_source,
                importerVersion: value.dxf_importer_version,
                version: value.version,
              }
            : {
                id: value.id,
...
function parseP2Workspace(
  ...
  const canvases = rows.canvases.map((row) => {
    ...
    return DrawingCanvasSchema.parse({
      ...
      background,
      ...(value.data.output_profile == null
        ? {}
        : { outputProfile: value.data.output_profile }),
      sortOrder: value.data.sort_order,
      version: value.data.version,
    });
  });
...
async function loadReviewEvidence(
  ...
  if (revision.status !== "review_requested" && revision.status !== "reviewed")
    return null;
...
async function loadDrawingWorkspaceWorkflowMetadata(
  client: DrawingWorkspaceClient,
  projectId: string,
  revision: DrawingRevisionRow,
) {
  const [issues, links, templateCandidates, reviewEvidence, checkpoints] =
    await Promise.all([
  ...
  return { issues, links, templateCandidates, reviewEvidence, checkpoints };
}
...
async function loadDrawingWorkspaceInternal(
  client: DrawingWorkspaceClient,
  ...
  if (Array.isArray(canvasProbe) && canvasProbe.length > 0) {
    if (mode === "shell") {
      const { issues, links, templateCandidates, reviewEvidence, checkpoints } =
        await loadDrawingWorkspaceWorkflowMetadata(client, projectId, revision);
      return {
        primarySource: file,
        templateCandidates,
        document: {
    ...
    const [
      ...
      loadAllDrawingRows(client, {
        ...
        select:
          "id,page_id,revision_id,project_id,name,space_kind,width_mm,height_mm,background_source_file_id,background_source_sha256,background_pdf_page,calibration,output_profile,sort_order,version",
      }),
      loadAllDrawingRows(client, {
      ...
      loadDrawingWorkspaceWorkflowMetadata(client, projectId, revision),
    ]);
    const { issues, links, templateCandidates, reviewEvidence, checkpoints } =
      workflow;
...
export function loadDrawingWorkspace(
  ...
  return loadDrawingWorkspaceInternal(client, input, "full");
}

/** Loads workflow metadata while the SHA-bound collaboration graph is fetched once. */
export function loadDrawingWorkspaceShell(
  client: DrawingWorkspaceClient,
  input: LoadDrawingWorkspaceInput,
) {
  return loadDrawingWorkspaceInternal(client, input, "shell");
}

/** Legacy revisions have no canvas graph and must keep their loaded entities. */
export function drawingWorkspaceUsesCanonicalGraph(
  workspace: DrawingWorkspace,
): boolean {
  return Array.isArray(workspace.document.revision.canvases);
}

function canonicalWorkspaceEntity(
  value: unknown,
...
function drawingEntitiesById<T extends { id: string }>(
  collection: string,
  values: readonly T[],
) {
  ...
  for (const value of values) {
    if (result[value.id])
      throw new Error(`Drawing ${collection} canonical IDs are duplicated.`);
...
/**
 * Grafts the one SHA-bound collaboration graph onto lightweight workflow
 * metadata for server-only source, estimate, and deep-link validation.
 */
export function hydrateDrawingWorkspaceShell(
  shell: DrawingWorkspace,
  bootstrap: DrawingWorkspaceCollaborationBootstrap,
  focus: Pick<
    LoadDrawingWorkspaceInput,
    "focusObjectId" | "focusEvidenceFileId"
  > = {},
): DrawingWorkspace {
  const revision = shell.document.revision;
  assertDrawingWorkspaceCollaborationBootstrapLineage(bootstrap, {
    documentId: shell.document.id,
    projectId: shell.document.project_id,
    revisionId: revision.id,
    revisionVersion: revision.version,
  });
  const graph = bootstrap.canonicalJson;
  if (graph.revision.sequence !== revision.sequence)
    throw new Error("Drawing canonical revision sequence is inconsistent.");
  if (bootstrap.revisionStatus !== revision.status)
    throw new DrawingWorkspaceRetryableError(
      "Drawing workflow status is inconsistent.",
    );
  const requiresReviewEvidence =
    revision.status === "review_requested" || revision.status === "reviewed";
  if (
    requiresReviewEvidence !== Boolean(revision.reviewEvidence) ||
    (revision.reviewEvidence &&
      (revision.reviewEvidence.subjectVersion !== revision.version ||
        revision.reviewEvidence.snapshotSha256 !== bootstrap.sha256))
  )
    throw new DrawingWorkspaceRetryableError(
      "Drawing review snapshot evidence is inconsistent.",
    );

  const pages = graph.pages.map((value) => DrawingPageSchema.parse(value));
  const canvases = graph.canvases.map((value) =>
    DrawingCanvasSchema.parse(value),
  );
  const layers = graph.layers.map((value) =>
    DrawingStructureLayerSchema.parse(
      canonicalWorkspaceEntity(value, ["pageId"]),
    ),
  );
  const objects = graph.objects.map((value) => {
    const object = DrawingObjectSchema.parse(
      canonicalWorkspaceEntity(value, ["lineageId", "pageId", "type"]),
    );
    if (
      !value ||
      typeof value !== "object" ||
      Array.isArray(value) ||
      (value as { type?: unknown }).type !== object.geometry.type
    )
      throw new Error("Drawing canonical object type is inconsistent.");
    return object;
  });
  const sources = graph.sources.map((value) =>
    DrawingObjectSourceSchema.parse(value),
  );
  const styles = graph.styles.map((value) =>
    DrawingStyleDefinitionSchema.parse(value),
  );
  const blocks = graph.blocks.map((value) => DrawingBlockSchema.parse(value));
  const blockInstances = graph.blockInstances.map((value) =>
    DrawingBlockInstanceSchema.parse(value),
  );
  const propertySchemas = graph.propertySchemas.map((value) =>
    DrawingPropertySchemaSchema.parse(value),
  );
  const propertyValues = graph.propertyValues.map((value) =>
    DrawingPropertyValueSchema.parse(value),
  );
  const tables = graph.tables.map((value) => DrawingTableSchema.parse(value));
  const structure = {
    pages: drawingEntitiesById("page", pages),
    canvases: drawingEntitiesById("canvas", canvases),
    layers: drawingEntitiesById("layer", layers),
    objects: drawingEntitiesById("object", objects),
    sources: drawingEntitiesById("source", sources),
    styles: drawingEntitiesById("style", styles),
    blocks: drawingEntitiesById("block", blocks),
    blockInstances: drawingEntitiesById("block instance", blockInstances),
    propertySchemas: drawingEntitiesById("property schema", propertySchemas),
    propertyValues: drawingEntitiesById("property value", propertyValues),
    tables: drawingEntitiesById("table", tables),
  };
  validateDrawingStructureState({ revisionId: revision.id, ...structure });

  const pageById = structure.pages;
  const canvasById = structure.canvases;
  for (const value of graph.layers) {
    if (!value || typeof value !== "object" || Array.isArray(value))
      throw new Error("Drawing canonical layer is invalid.");
    const layer = structure.layers[(value as { id: string }).id];
    const canvas = layer ? canvasById[layer.canvasId] : null;
    if (!canvas || (value as { pageId?: unknown }).pageId !== canvas.pageId)
      throw new Error("Drawing canonical layer page is inconsistent.");
  }
  for (const value of graph.objects) {
    if (!value || typeof value !== "object" || Array.isArray(value))
      throw new Error("Drawing canonical object is invalid.");
    const object = structure.objects[(value as { id: string }).id];
    const layer = object ? structure.layers[object.layerId] : null;
    const canvas = layer ? canvasById[layer.canvasId] : null;
    if (!canvas || (value as { pageId?: unknown }).pageId !== canvas.pageId)
      throw new Error("Drawing canonical object page is inconsistent.");
  }
  for (const canvas of canvases) {
    if (
      canvas.background &&
      (!shell.primarySource ||
        canvas.background.sourceFileId !== shell.primarySource.id ||
        canvas.background.sourceSha256 !== shell.primarySource.sha256)
    )
      throw new Error("Drawing canvas source evidence is inconsistent.");
  }
  ...
  const orderedPages = ordered(pages, (page) => page.sortOrder);
  const focusedObject = focus.focusObjectId
    ? structure.objects[focus.focusObjectId]
    : null;
  if (focus.focusObjectId && !focusedObject)
    throw new DrawingWorkspaceConflictError(
      "연결된 도면 근거를 열 수 없습니다.",
    );
  const focusedLayer = focusedObject
    ? structure.layers[focusedObject.layerId]
    : null;
  const focusedCanvas = focusedLayer
    ? structure.canvases[focusedLayer.canvasId]
    : null;
  if (focusedObject && (!focusedLayer || !focusedCanvas))
    throw new DrawingWorkspaceConflictError(
      "연결된 도면 근거를 열 수 없습니다.",
    );
  const focusedEvidence = focus.focusEvidenceFileId
    ? sources.filter(
        (source) =>
          source.objectId === focus.focusObjectId &&
          source.sourceFileId === focus.focusEvidenceFileId,
      )
    : [];
  if (focus.focusEvidenceFileId && focusedEvidence.length !== 1)
    throw new DrawingWorkspaceConflictError(
      "연결된 도면 근거를 열 수 없습니다.",
    );
  const focusedPdfEvidence = focusedEvidence.find(
  ...
  if (
    focusedPdfEvidence &&
    (!focusedBackground ||
      focusedBackground.sourceFileId !== focus.focusEvidenceFileId ||
      focusedBackground.pdfPageNumber !== focusedPdfEvidence.pdfPageNumber)
  )
    throw new DrawingWorkspaceConflictError(
      "연결된 도면 근거를 열 수 없습니다.",
  ...
  const activeCanvas =
    ...
    (activePage
      ...
      : null);
  if (!activePage || !activeCanvas)
    throw new Error("Drawing canonical default canvas is missing.");

  const objectIds = new Set(objects.map((object) => object.id));
  const issueIds = new Set(revision.issues.map((issue) => issue.id));
  const canonicalIssuePairs = graph.issues.map((value) => {
    const issue = CollaborationCanonicalIssueSchema.parse(value);
    if (!objectIds.has(issue.objectId) || !issueIds.has(issue.id))
      throw new DrawingWorkspaceRetryableError(
        "Drawing issue workflow is inconsistent.",
      );
    return `${issue.id}:${issue.objectId}`;
  });
  const workflowIssuePairs = revision.issueLinks.map((link) => {
    if (
      link.project_id !== shell.document.project_id ||
      link.revision_id !== revision.id ||
      !objectIds.has(link.object_id) ||
      !issueIds.has(link.issue_id)
    )
      throw new DrawingWorkspaceRetryableError(
        "Drawing issue workflow is inconsistent.",
      );
    return `${link.issue_id}:${link.object_id}`;
  });
  const canonicalIssueSet = new Set(canonicalIssuePairs);
  const workflowIssueSet = new Set(workflowIssuePairs);
  if (
    canonicalIssueSet.size !== canonicalIssuePairs.length ||
    workflowIssueSet.size !== workflowIssuePairs.length ||
    canonicalIssueSet.size !== workflowIssueSet.size ||
    [...canonicalIssueSet].some((pair) => !workflowIssueSet.has(pair))
  )
    throw new DrawingWorkspaceRetryableError(
      "Drawing issue workflow is inconsistent.",
    );
    ...
    document: {
      ...
      revision: {
        ...
        pages: orderedPages.map((page) => ({
          ...
          layers: ordered(
            layers.filter(
              (layer) => canvasById[layer.canvasId]?.pageId === page.id,
            ),
            (layer) => layer.sortOrder,
          ),
          objects: ordered(
            objects.filter(
              (object) =>
                canvasById[structure.layers[object.layerId]?.canvasId]
                  ?.pageId === page.id,
            ),
            () => 0,
          ),
          blockInstances: ordered(
            blockInstances.filter(
              (instance) =>
                canvasById[structure.layers[instance.layerId]?.canvasId]
                  ?.pageId === page.id,
            ),
            () => 0,
          ),
        })),
        canvases: ordered(canvases, (canvas) => canvas.sortOrder),
        layers: ordered(layers, (layer) => layer.sortOrder),
        objects: ordered(objects, () => 0),
        sources: ordered(sources, () => 0),
        styles: ordered(styles, () => 0),
        blocks: ordered(blocks, () => 0),
        blockInstances: ordered(blockInstances, () => 0),
        propertySchemas: ordered(propertySchemas, () => 0),
        propertyValues: ordered(propertyValues, () => 0),
        tables: ordered(tables, () => 0),
        issueLinks: revision.issueLinks.filter(
          (link) =>
            objectIds.has(link.object_id) && issueIds.has(link.issue_id),
        ),
      },
...
const DrawingWorkspaceAttachFileSchema = z
  .object({
    id: Uuid,
    project_id: Uuid,
    kind: z.literal("pdf"),
    original_filename: z.string().min(1).max(1_024),
    byte_size: z.number().int().safe().nonnegative(),
    sha256: Sha256,
...
const DrawingWorkspaceAttachDocumentSchema = z
  .object({
    id: Uuid,
    project_id: Uuid,
...
/** Lists immutable PDFs that are not already the primary source of a workspace. */
export async function loadDrawingWorkspaceAttachCandidates(
  client: DrawingWorkspaceClient,
  projectId: string,
): Promise<DrawingWorkspaceSourceAttachCandidate[]> {
  const parsedProjectId = Uuid.parse(projectId);
  const [fileRows, documentRows] = await Promise.all([
    loadAllDrawingRows<Record<string, unknown> & { id: string }>(client, {
      ...
      select:
        "id,project_id,kind,original_filename,byte_size,sha256,immutable,created_at",
    }),
    loadAllDrawingRows<Record<string, unknown> & { id: string }>(client, {
      table: "lukas_drawing_documents",
      projectId: parsedProjectId,
  ...
  const files = fileRows.map((row) => {
    const parsed = DrawingWorkspaceAttachFileSchema.safeParse(row);
    if (
      !parsed.success ||
      parsed.data.project_id !== parsedProjectId ||
      parsed.data.original_filename.trim().length === 0
    )
      throw new Error("Drawing PDF attachment catalog is invalid.");
    return parsed.data;
  });
  const documents = documentRows.map((row) => {
    const parsed = DrawingWorkspaceAttachDocumentSchema.safeParse(row);
    if (!parsed.success || parsed.data.project_id !== parsedProjectId)
      throw new Error("Drawing PDF attachment catalog is invalid.");
    return parsed.data;
  });
  const usedSourceIds = new Set(
    documents.flatMap((document) =>
      document.source_file_id ? [document.source_file_id] : [],
    ),
  );
  return files
    .filter((file) => !usedSourceIds.has(file.id))
    .map((file) => ({
      id: file.id,
      kind: "pdf" as const,
      originalFilename: file.original_filename,
...
const IfcDerivativeJobStatusSchema = z
  .object({
    state: z.enum([
      "queued",
      "processing",
...
async function loadIfcDerivativeJobState(
  client: DrawingWorkspaceClient,
  file: Pick<DrawingWorkspaceFile, "id" | "sha256">,
) {
  const result = await client.rpc("lukas_drawing_ifc_derivative_job_status", {
    p_source_file_id: file.id,
    p_source_sha256: file.sha256,
  });
  if (result.error)
    throw new Response("IFC derivative 작업 상태를 확인하지 못했습니다.", {
      status: 500,
    });
  if (result.data === null) return null;
  const parsed = IfcDerivativeJobStatusSchema.safeParse(result.data);
  if (!parsed.success)
    throw new Response("IFC derivative 작업 상태가 올바르지 않습니다.", {
      status: 409,
    });
  return parsed.data;
}
...
async function validateSelfContainedGlb(
  ...
  if (
    !Number.isSafeInteger(document.scene) ||
    (document.scene as number) < 0 ||
    (document.scene as number) >= scenes.length
  )
    throw new Error("GLB default scene is unavailable");
  const scene = scenes[document.scene as number] as Record<string, unknown>;
  if (!Array.isArray(scene.nodes))
    throw new Error("GLB default scene roots are unavailable");
  ...
  while (pendingNodes.length > 0) {
    ...
    if (
      ...
      throw new Error("GLB scene node is unavailable");
    if (reachableNodes.has(nodeIndex as number)) continue;
    reachableNodes.add(nodeIndex as number);
    const node = nodes[nodeIndex as number] as Record<string, unknown>;
    if (node.children === undefined) continue;
    if (!Array.isArray(node.children))
      throw new Error("GLB node children are invalid");
  ...
  >();
  const referencedMeshIndices = new Set<number>();
  for (const [nodeIndex, value] of nodes.entries()) {
    ...
    if (!reachableNodes.has(nodeIndex))
      throw new Error("GLB mesh node is outside the default scene");
    referencedMeshIndices.add(node.mesh as number);
  ...
  if (referencedMeshIndices.size !== meshes.length)
    throw new Error("GLB mesh is not referenced by the default scene");
  ...
  for (const [nodeId, resolved] of nodesById) {
    ...
    for (
      ...
      const primitive = primitives[primitiveIndex] as Record<string, unknown>;
      const mode = primitive.mode ?? 4;
      if (mode !== 1 && mode !== 4)
        throw new Error("GLB primitive mode is unsupported");
...
export async function loadDrawingIfcDerivative(
  ...
  if (!candidate) {
    ...
    const job = await loadIfcDerivativeJobState(client, file);
    if (job) return noIfcDerivative(file.sha256, job.state);
...
export async function loadDrawingWorkspaceSourceBundle(
  ...
  const primarySource = workspace.primarySource;
  const projectId = workspace.document.project_id;
  ...
  const selectedId =
    selectedIfcFileId ??
    (primarySource?.kind === "ifc" ? primarySource.id : null);
  ...
  if (
    primarySource &&
    (!validFile(primarySource) ||
      primarySource.kind === "dxf" ||
      primarySource.kind === "dwg")
  )
  ...
  if (primarySource && !catalog.some((file) => file.id === primarySource.id))
    catalog.push(drawingWorkspaceSourceCatalogItem(primarySource));
  const workspaceRevision = workspace.document.revision;

  const revisionEdges =
    primarySource?.kind === "pdf"
      ? await loadAllDrawingRows<DrawingFileRevisionEdgeRow>(client, {
          table: "lukas_qto_file_revisions",
          projectId,
          filters: [
            ["current_file_id", primarySource.id],
  ...
  if (
    revisionEdge &&
    (!primarySource ||
      revisionEdge.project_id !== projectId ||
      revisionEdge.current_file_id !== primarySource.id ||
      revisionEdge.current_sha256 !== primarySource.sha256 ||
  ...
  if (
    primarySource &&
    ((backgroundPage &&
      (backgroundPage.background_source_file_id !== primarySource.id ||
        backgroundPage.background_source_sha256 !== primarySource.sha256)) ||
      (p2Background &&
        (p2Background.sourceFileId !== primarySource.id ||
          p2Background.sourceSha256 !== primarySource.sha256)))
  )
    throw new Response("도면 배경 원본 증거가 일치하지 않습니다.", {
      status: 409,
    });
  ...
  const primaryLoaded =
    primarySource?.kind === "ifc"
      ? loadSelectedIfc
      : Boolean(primarySource && (backgroundPage || p2Background));
  let unavailable: DrawingWorkspaceSourceUnavailableError | null = null;
  let primary: DrawingWorkspaceSourceBundle["primary"] = primarySource
    ? drawingWorkspaceSourceCatalogItem(primarySource)
    : null;
  let pdf: DrawingWorkspacePdfSourceDescriptor | null = null;
  let ifc: DrawingWorkspaceIfcSourceDescriptor | null = null;
  if (primarySource && primaryLoaded) {
    try {
      const loaded = await loadSource(primarySource);
      primary = loaded;
      if (loaded.kind === "pdf") pdf = loaded;
    } catch (error) {
      if (!(error instanceof DrawingWorkspaceSourceUnavailableError))
        throw error;
      unavailable = error;
    }
  }
  if (selectedCandidate && loadSelectedIfc) {
    if (selectedCandidate.id === primarySource?.id) {
      ifc =
        (signed.get(
          selectedCandidate.id,
        ) as DrawingWorkspaceIfcSourceDescriptor) ?? null;
    } else {
      try {
        ifc = (await loadSource(
          selectedCandidate,
...
export class DrawingWorkspaceSourceUnavailableError extends Error {
  readonly recoveryBundle: DrawingWorkspaceSourceBundle | null;

  constructor(
    message: string,
    recoveryBundle: DrawingWorkspaceSourceBundle | null = null,
  ) {
    super(message);
    this.name = "DrawingWorkspaceSourceUnavailableError";
    this.recoveryBundle = recoveryBundle;
    Object.defineProperty(this, "recoveryBundle", { enumerable: false });
  }
}
...
const drawingRetryableCodes = new Set(["40001", "40P01", "P1T01"]);
...
export { rpcResult as drawingWorkspaceRpcResult };
...
const DrawingWorkspaceSourceAttachInputSchema = z
  .object({
    documentId: Uuid,
    revisionId: Uuid,
    canvasId: Uuid,
...
const DrawingWorkspaceSourceAttachResultSchema = z
  .object({
    documentId: Uuid,
    revisionId: Uuid,
    canvasId: Uuid,
...
/** Attaches one immutable PDF and its page-one canvas background atomically. */
export async function attachDrawingWorkspaceSource(
  client: DrawingWorkspaceClient,
  input: z.infer<typeof DrawingWorkspaceSourceAttachInputSchema>,
) {
  const parsed = DrawingWorkspaceSourceAttachInputSchema.parse(input);
  const { data, error } = await client.rpc("lukas_drawing_attach_source", {
    p_document_id: parsed.documentId,
    p_revision_id: parsed.revisionId,
    p_canvas_id: parsed.canvasId,
    p_source_file_id: parsed.sourceFileId,
    p_request_id: parsed.requestId,
  });
  const acknowledgement = DrawingWorkspaceSourceAttachResultSchema.safeParse(
    rpcResult(data, error),
  );
  if (!acknowledgement.success)
    throw new DrawingWorkspaceRpcError(
      "도면 원본 연결 확인 응답이 올바르지 않습니다.",
    );
  const result = acknowledgement.data;
  const versionIds = Object.keys(result.resultVersions);
  if (
    result.documentId !== parsed.documentId ||
    result.revisionId !== parsed.revisionId ||
    result.canvasId !== parsed.canvasId ||
    result.sourceFileId !== parsed.sourceFileId ||
    versionIds.length !== 1 ||
    versionIds[0] !== parsed.canvasId
  )
    throw new DrawingWorkspaceRpcError(
      "도면 원본 연결 확인 응답이 요청 대상과 일치하지 않습니다.",
    );
  return result;
}
...
const DrawingOperationDispositionSchema = z
  .object({
    clientOperationId: Uuid,
  ...
  .superRefine((value, context) => {
    if (
      (value.status === "acked" && value.authoritativeSequence === null) ||
      (value.status === "rejected" && value.authoritativeSequence !== null)
    )
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["authoritativeSequence"],
        message: "도면 작업 폐기 결과의 순서가 올바르지 않습니다.",
      });
    if (
      value.status === "rejected" &&
      Object.keys(value.resultVersions).length > 0
    )
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["resultVersions"],
        message: "폐기된 도면 작업에는 결과 버전이 없어야 합니다.",
      });
  });

export type DrawingOperationDisposition = z.infer<
  typeof DrawingOperationDispositionSchema
>;

export async function discardDrawingOperationSuffix(
  client: DrawingWorkspaceClient,
  revisionId: string,
  inputs: DrawingOperationInput[],
) {
  const parsedRevisionId = Uuid.parse(revisionId);
  if (
    inputs.length < 1 ||
    inputs.length > MAX_DRAWING_CONFLICT_DISCARD_OPERATIONS
  )
    throw new DrawingWorkspaceRpcError(
      "폐기할 충돌 작업 범위가 올바르지 않습니다.",
    );
  const operations = inputs.map(parseOperation);
  if (operations.some((operation) => operation.revisionId !== parsedRevisionId))
    throw new DrawingWorkspaceRpcError(
      "폐기할 작업의 도면 리비전이 일치하지 않습니다.",
    );
  const { data, error } = await client.rpc(
  ...
  const result = z
    .object({
      dispositions: z
        .array(DrawingOperationDispositionSchema)
        .min(1)
        .max(MAX_DRAWING_CONFLICT_DISCARD_OPERATIONS),
    })
    .strict()
    .parse(rpcResult(data, error));
  if (
    result.dispositions.length !== operations.length ||
    result.dispositions.some(
      (disposition, index) =>
        disposition.clientOperationId !== operations[index].clientOperationId,
    )
  )
    throw new DrawingWorkspaceRpcError(
      "도면 작업 폐기 응답이 요청 순서와 일치하지 않습니다.",
    );
  for (const [index, disposition] of result.dispositions.entries()) {
    if (disposition.status !== "acked") continue;
    const expected = expectedOperationResultVersions(operations[index]);
    const ids = Object.keys(expected);
    if (
      Object.keys(disposition.resultVersions).length !== ids.length ||
      ids.some(
        (id) =>
          !matchesOperationResultVersion(
            expected[id],
            disposition.resultVersions[id],
          ),
      )
    )
      throw new DrawingWorkspaceRpcError(
        "이미 저장된 도면 작업의 결과 버전이 일치하지 않습니다.",
      );
  }
  return result;
}
...
export function expectedOperationResultVersions(
...
export async function handleWorkspaceMutation({
  ...
  requestReviewWithoutCollaboration = requestDrawingReview,
  ...
  requestReviewWithoutCollaboration?: typeof requestDrawingReview;
  ...
  try {
    ...
    if (mutation.intent === "record_revision_decision") {
      ...
      if (mutation.intent === "create_from_template") {
        ...
        if (actorId && collaborationEnabled) {
      ...
      } else if (mutation.intent === "discard_conflicted_operations") {
        assertDraftWorkspace(workspace);
        assertCurrentWorkspaceRevision(workspace, mutation.revisionId);
        result = await discardDrawingOperationSuffix(
          client,
          mutation.revisionId,
          mutation.operations,
        );
        if (actorId && collaborationEnabled) {
          const dispositions = (
            result as {
              dispositions: DrawingOperationDisposition[];
            }
          ).dispositions;
          for (const [index, disposition] of dispositions.entries()) {
            ...
            try {
              delivered = await deliverOutcome({
                actorId,
                projectId,
                operation: mutation.operations[index],
                outcome: disposition.status === "acked" ? "acked" : "rejected",
                authoritativeSequence: disposition.authoritativeSequence,
                resultVersions: disposition.resultVersions,
              });
            } catch {
              // The disposition RPC is idempotent, so a later retry can finish
              // the immutable collaboration receipts without changing DB state.
            }
            if (!delivered)
              throw new DrawingWorkspaceRetryableError(
                "충돌 작업 정리 결과 전달을 다시 시도합니다.",
              );
          }
        }
        ...
        result = collaborationEnabled
          ...
          : await requestReviewWithoutCollaboration(
              client,
              mutation.revisionId,
            );
    ...
    if (
      ...
      try {
        const delivered = await deliverOutcome({
          ...
          // A database version conflict is a definitive non-application. Keep
          // the HTTP conflict UX locally, but exclude it from the shared
          // projection so the immutable collaboration ledger can be resolved.
          outcome: "rejected",
        });
        if (!delivered)
          return {
