...
cmake_bin_sha=86a5ad068abe889c8d344089eb805bb70012661f1f57c16d5118d14e022f73dc
...
case "$([ -f "$cmake_archive" ] && realpath "$cmake_archive"):$([ -f "$cmake_sha_file" ] && realpath "$cmake_sha_file")" in
  /private/tmp/*:/private/tmp/*) ;;
  *) echo "CMake archive and SHA file must stay under /private/tmp" >&2; exit 1 ;;
esac
...
mkdir -p "$build_dir"
toolchain_dir=$(mktemp -d "$build_dir/.cmake-toolchain.XXXXXX")
trap 'rm -rf -- "$toolchain_dir"' EXIT HUP INT TERM
cmake_bin="$toolchain_dir/cmake-4.4.3-macos-universal/CMake.app/Contents/bin/cmake"
tar -xzf "$cmake_archive" -C "$toolchain_dir"
require_equal "$(shasum -a 256 "$cmake_bin" | awk '{print $1}')" "$cmake_bin_sha" "extracted CMake binary SHA-256"
