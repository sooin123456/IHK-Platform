...
namespace Lukas.Qto.Core.SelfTest
{
    internal static class Program
    {
        ...
        private static void TestRebarTakeoff()
        {
            ...
            string reportDirectory = Path.Combine(Path.GetTempPath(), "lukas-rebar-report-" + Guid.NewGuid().ToString("N"));
        ...
        {
            string directory = Path.Combine(Path.GetTempPath(), "lukas-element-ledger-" + Guid.NewGuid().ToString("N"));
        ...
        {
            string directory = Path.Combine(Path.GetTempPath(), "lukas-export-package-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(directory);
            try
            {
        ...
        private static void TestConcreteTakeoff()
        {
            ...
            var overDeducted = ConcreteTakeoff.Calculate(new[] { Revit("OD1", 1m, StructuralQuantityOperation.Normal, "301"), Revit("OD2", -2m, StructuralQuantityOperation.Deduction, "302") }, bundle, registry);
            Assert(overDeducted.Status == StructuralQuantityStatus.FAIL && !overDeducted.Rows.Single().FinalQuantityM3.HasValue,
                "deduction cannot exceed the normal concrete quantity and still produce a pass");
            ConcreteInputRow validOtherGroup = Revit("OD3", 1m, StructuralQuantityOperation.Normal, "303");
            validOtherGroup.Floor = "2F";
            var mixedFailure = ConcreteTakeoff.Calculate(new[] { Revit("OD4", 1m, StructuralQuantityOperation.Normal, "304"), Revit("OD5", -2m, StructuralQuantityOperation.Deduction, "305"), validOtherGroup }, bundle, registry);
            Assert(mixedFailure.Status == StructuralQuantityStatus.FAIL,
                "an invalid concrete group cannot be hidden by another pass or not-evaluated group");
        ...
        {
            string directory = Path.Combine(Path.GetTempPath(), "lukas-takeoff-report-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(directory);
            try
            {
                var takeoff = new ConcreteTakeoffResult { Status = StructuralQuantityStatus.PASS, Rows = new[] { new ConcreteTakeoffRow {
                    ...
                    FinalQuantityM3 = 12345678901234567891.35802467m, Formula = "ROUND(raw*(1+rate),8)", RuleId = "RULE-1", RuleHash = new string('A', 64),
                ...
                Assert(row.RecordType == "TAKEOFF" && row.Status == "PASS" && row.SourceKind == "Zg" && row.Building == "=어린이집" && row.Floor == "  @1F" && row.Member == "'W1" &&
                    row.RawM3 == 12345678901234567890.12345678m && row.DeductionM3 == -0.00000001m && row.AllowanceM3 == 1.23456789m && row.FinalM3 == 12345678901234567891.35802467m &&
                ...
                var invalidArithmetic = new ConcreteTakeoffResult { Status = StructuralQuantityStatus.PASS, Rows = new[] { new ConcreteTakeoffRow {
                    SourceKind = ConcreteSourceKind.Revit, Building = "A", Floor = "1F", Member = "Wall", Spec = "25-270-15", Status = StructuralQuantityStatus.PASS,
                    RawQuantityM3 = 10m, DeductionQuantityM3 = -1m, AllowanceQuantityM3 = 5m, FinalQuantityM3 = 12m,
                    Formula = "forged", RuleId = "RULE-X", RuleHash = new string('c', 64), RuleSource = "rules.csv!2",
                    ElementIds = new[] { "901" }, Message = "forged" } } };
                bool invalidArithmeticRejected = false;
                try { TakeoffReport.WriteTakeoff(Path.Combine(directory, "invalid-arithmetic.csv"), invalidArithmetic, takeoffInputs); } catch (InvalidDataException) { invalidArithmeticRejected = true; }
                Assert(invalidArithmeticRejected && !File.Exists(Path.Combine(directory, "invalid-arithmetic.csv")),
                    "takeoff writer recalculates final minus net as allowance before publishing an artifact");
                ...
                var reconciliation = new ConcreteReconciliationResult { Status = StructuralQuantityStatus.REVIEW, DeltasClose = true, LeftTotalM3 = 1m, RightTotalM3 = 1.5m, TotalDeltaM3 = 0.5m, RowDeltaSumM3 = 0.5m,
                    Rows = new[] { new ConcreteReconciliationRow { CommonKey = string.Join("\u001f", "A", "1F", "W", "25-270-15"), Status = StructuralQuantityStatus.REVIEW,
        ...
        {
            string directory = Path.Combine(Path.GetTempPath(), "lukas-formwork-ledger-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(directory);
            try
            {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "lukas-self-test.xlsx");
            string noCachePath = Path.Combine(Path.GetTempPath(), "lukas-generic-no-cache-self-test.xlsx");
            string numericIdPath = Path.Combine(Path.GetTempPath(), "lukas-generic-numeric-id-self-test.xlsx");
            try
            {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "lukas-ems-self-test.xlsx");
            try
            {
                using (var archive = ZipFile.Open(path, ZipArchiveMode.Create))
                {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "lukas-ems-invalid-self-test.xlsx");
            string localePath = Path.Combine(Path.GetTempPath(), "lukas-ems-locale-self-test.xlsx");
            string textPath = Path.Combine(Path.GetTempPath(), "lukas-ems-text-number-self-test.xlsx");
            string noCachePath = Path.Combine(Path.GetTempPath(), "lukas-ems-no-cache-self-test.xlsx");
            string sharedNoCachePath = Path.Combine(Path.GetTempPath(), "lukas-ems-shared-no-cache-self-test.xlsx");
            string invalidQuantityPath = Path.Combine(Path.GetTempPath(), "lukas-ems-invalid-quantity-self-test.xlsx");
            string unsafeMagnitudePath = Path.Combine(Path.GetTempPath(), "lukas-ems-unsafe-magnitude-self-test.xlsx");
            try
            {
                using (var archive = ZipFile.Open(path, ZipArchiveMode.Create))
                {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "lukas-ems-summary-self-test.xlsx");
            string duplicatePath = Path.Combine(Path.GetTempPath(), "lukas-ems-summary-duplicate-self-test.xlsx");
            try
            {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "lukas-self-test.csv");
            try
            {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "lukas-qto-traceability.csv");
        ...
        {
            string template = Path.Combine(Path.GetTempPath(), "lukas-mapping-template.xlsx");
            string index = Path.Combine(Path.GetTempPath(), "lukas-qto-index.xlsx");
            try
            {
        ...
        {
            string input = Path.Combine(Path.GetTempPath(), "=lukas-manifest-input.csv");
            string manifest = Path.Combine(Path.GetTempPath(), "lukas-manifest.csv");
            try
            {
                ...
                Assert(rows.Any(row => row[0] == "규칙버전" && row[1] == RunManifest.RulesetVersion) && rows.Any(row => row[0] == "엔진_코어_SHA256" && row[1].Length == 64) && rows.Any(row => row[0] == "QTO_SHA256" && row[1].Length == 64) && rows.Any(row => row[0] == "QTO_파일" && row[1] == "'=lukas-manifest-input.csv") && rows.Any(row => row[0] == "수량허용오차" && row[1] == "0.01") && rows.Any(row => row[0] == "KRW허용오차" && row[1] == "500") && rows.Any(row => row[0] == "소스게이트" && row[1] == "SKIPPED"), "run manifest hashes, engine identity, spreadsheet-safe filename, policy, and gate state");
            }
            finally
            {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "lukas-report.html");
            try
            {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "lukas-report-precision.csv");
            try
            {
