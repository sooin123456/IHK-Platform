...
using Lukas.Qto.Commands;
using Lukas.Qto.Compat;
using Lukas.Qto.Core;

namespace Lukas.Qto.RevitStubTest
{
    internal static class Program
    {
        private static int Main()
        {
            try
            {
                ...
                document.Add(first);
                document.Add(second);
                document.Add(Noise(103, BuiltInCategory.OST_Lines, "<Sketch>", BuiltInParameter.CURVE_ELEM_LENGTH, 999));
                document.Add(Noise(104, BuiltInCategory.OST_Materials, "Materials", BuiltInParameter.HOST_AREA_COMPUTED, 999));
                document.Add(Noise(105, BuiltInCategory.OST_LegendComponents, "Legend Components", BuiltInParameter.HOST_VOLUME_COMPUTED, 999));

                var extractionCounts = new RevitExtractionCounts();
                var rows = QuantityExtractor.Extract(document, extractionCounts);
                ...
                Assert(extractionCounts.ExcludedNonQuantityCount == 3 && extractionCounts.EligibleHostModelCount == 2,
                    "sketches, materials, and legend helpers are excluded by the shared physical-category filter");
        ...
        private static void TestFormworkFaceLedger()
        {
            ...
            var wall = new Element { Id = Id(602), TypeId = typeId, Category = ModelCategory("Walls", BuiltInCategory.OST_Walls), Name = "Concrete Wall" };
        ...
        private static void TestElementQuantityLedger()
        {
            ...
            var missing = new Element { Id = Id(203), TypeId = typeId, LevelId = levelId, Category = ModelCategory("Walls", BuiltInCategory.OST_Walls), Name = "Wall" };
        ...
        private static void TestPropertiesScopesAndEvidence()
        {
            ...
            var link = new RevitLinkInstance { Id = Id(503), TypeId = typeId, Category = ModelCategory("Revit Links", BuiltInCategory.OST_GenericModel), Name = "Linked model" };
            ...
            try
            {
                ...
                Assert(result == Result.Succeeded && ElementQuantityLedger.Read(path).Count == 1 && evidence.Contains("\"schema_version\": \"revit-properties-evidence-v2\"") &&
                    evidence.Contains("\"physical_category_filter\": \"built-in-category-physical-v1\"") && evidence.Contains("\"revit_version\": \"2025\"") && evidence.Contains("\"revit_build\": \"2025.1.2\"") &&
                    evidence.Contains("\"scope\": \"Selection\"") && evidence.Contains("\"excluded_link_instances\": 1") && evidence.Contains("\"excluded_non_quantity\": 0") && evidence.Contains("\"csv_rows\": 1"),
                    "Properties command writes physical-filter version, Revit build, scope, and exclusion counts without changing CSV contract");
        ...
        private static void AssertInvalidVolume(double value, string name)
        {
            ...
            var element = new Element { Id = Id(204), TypeId = typeId, Category = ModelCategory("Walls", BuiltInCategory.OST_Walls), Name = "Wall" };
        ...
        {
            var element = new Element { Id = Id(id), TypeId = typeId, LevelId = levelId, Category = ModelCategory("  Walls  ", BuiltInCategory.OST_Walls), Name = "Wall" };
        ...
        private static Category ModelCategory(string name, BuiltInCategory category) { return new Category { Id = Id((int)category), Name = name, CategoryType = CategoryType.Model }; }
        private static Element Noise(long id, BuiltInCategory category, string name, BuiltInParameter parameter, double value)
        {
            var element = new Element { Id = Id(id), Category = ModelCategory(name, category), Name = name };
            element.SetParameter(parameter, Number(value));
            return element;
        }
        ...
        private static void TestIfcQtoPackage(Document document)
        {
            ...
            try
            {
                ...
                var invalidVolumeElement = new Element { Id = Id(501), TypeId = invalidTypeId, Category = ModelCategory("Walls", BuiltInCategory.OST_Walls), Name = "Wall" };
