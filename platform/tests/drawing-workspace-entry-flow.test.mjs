...
test("workspace preview renders without the marketing navigation and footer", async () => {
  const topLevelPreviewRoute = routes.find(
    (route) => route.file === "lukas/screens/workspace-preview.tsx",
  );
  const nestedPreviewRoute = navigationRoute?.children?.find(
    (route) => route.file === "lukas/screens/workspace-preview.tsx",
  );
  const previewRoute = topLevelPreviewRoute ?? nestedPreviewRoute;
  assert.ok(previewRoute);

  let html;
  if (nestedPreviewRoute) {
    html = await renderNavigationRoute({
      leaf: nestedPreviewRoute,
      url: nestedPreviewRoute.path,
    });
  } else {
    ...
    const router = createMemoryRouter(
      [
        {
          id: previewRoute.id ?? previewRoute.file,
          path: previewRoute.path,
          Component: PreviewSentinel,
        },
      ],
      { initialEntries: [previewRoute.path] },
    );
...
test("workspace dashboard resumes documents canonically and routes project cards to drawing lists", () => {
  ...
  assert.match(
    emptyHtml,
    /href="\/projects\/00000000-0000-4000-8000-000000000001\/drawings"/,
  );
  assert.doesNotMatch(emptyHtml, /\/workspaces\/new/);
  assert.match(emptyHtml, /도면 없음/);
  assert.match(
    previewHtml,
    /href="\/workspace-preview\?project=00000000-0000-4000-8000-000000000001"/,
  );
});
...
test("workspace dashboard renders Korean calendar dates identically across server timezones", () => {
  ...
  try {
    ...
    assert.deepEqual(calendarDates(utcHtml), ["8월 31일", "8월 31일"]);
...
test("dashboard exposes one quick-start disclosure and a real resume link", () => {
  ...
  assert.equal(html.match(/<summary[^>]*>.*?새 작업/s)?.length, 1);
  assert.match(html, /빈 도면 시작/);
  assert.match(html, /템플릿 선택/);
  assert.match(html, /파일 열기/);
  ...
  assert.match(
    html,
    new RegExp(`href="/projects/${fixture.projects[0].id}/drawings"`),
  );
  assert.doesNotMatch(html, /최근 도면|이전 도면 보기/);
});

test("workspace preview uses safe quick-start links without production forms", () => {
  ...
  assert.match(html, /<summary[^>]*>.*?새 작업/s);
  assert.match(html, /href="\/workspace-preview\?state=default&amp;start=blank"/);
  assert.match(html, /href="\/workspace-preview\?state=default&amp;start=template"/);
  assert.match(html, /href="\/workspace-preview\?state=default&amp;start=file"/);
...
test("workspace loader renders only drawing document IDs as canonical dashboard destinations", async () => {
  ...
  assert.match(
    html,
    new RegExp(`href="/projects/${fixture.projectWithFileOnly}/drawings"`),
  );
  assert.doesNotMatch(
    html,
    new RegExp(`/projects/${fixture.projectWithFileOnly}/workspaces/new`),
  );
  assert.doesNotMatch(
    html,
    new RegExp(
...
test("workspace dashboard renders one active space menu with an accessible selector and drawer", () => {
  ...
  assert.match(
    html,
    new RegExp(`href="/projects/${fixture.projects[1].id}/drawings"`),
  );
  assert.match(html, /내 개인 공간의 기본 프로젝트에 안전하게 저장/);
});
