...
function fileMatchesKind(kind: string, filename: string) {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!fileMatchesKind(input.kind, input.originalFilename))
      throw new HttpError("선택한 자료 종류와 파일 형식이 맞지 않습니다.", 400);
    ...
    if (existingFile) throw new HttpError("이미 등록된 원본 파일입니다.", 409);
    ...
    const storedContentType =
      typeof (storedObject as { contentType?: unknown }).contentType ===
        "string" &&
      (storedObject as { contentType: string }).contentType.length <= 255
        ? (storedObject as { contentType: string }).contentType
        : input.contentType;
    ...
    if (verificationError || !verification) {
      ...
      const { data: existingVerification, error: existingVerificationError } =
        await admin
          .from("lukas_qto_verified_uploads")
          .select(
            "id, actor_id, byte_size, consumed_file_id, content_type, expires_at, kind, original_filename, project_id, sha256, storage_path",
          )
          .eq("storage_path", storagePath)
          .maybeSingle();
      const sameEvidence =
      ...
      if (Date.parse(existingVerification.expires_at) <= Date.now()) {
        const { data: refreshedVerification, error: refreshError } = await admin
          .from("lukas_qto_verified_uploads")
          .update({
            expires_at: new Date(
