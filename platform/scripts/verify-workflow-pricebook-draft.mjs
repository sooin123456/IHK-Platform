...
const browser=await chromium.launch();try{
 ...
 await page.reload({waitUntil:'networkidle'});await expect(mapping.getByLabel('코드 열',{exact:true})).toHaveValue('2');await expect(mapping.getByLabel('출처 열',{exact:true})).toHaveValue('3');
 ...
 await mapping.getByRole('button',{name:'열 연결 적용',exact:true}).click();await page.reload({waitUntil:'networkidle'});await expect(section.getByLabel('단가 CSV 내용',{exact:true})).toContainText('P-01');await section.getByRole('button',{name:'가져오기 미리보기',exact:true}).click();await expect(section.getByRole('region',{name:'단가 가져오기 미리보기'})).toContainText('배관, 일반');
