...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (form.get("intent") !== "mark_read")
    return data({ ok: false, error: "지원하지 않는 요청입니다." }, { status: 400, headers });
  const id = form.get("notification_id");
  if (typeof id !== "string" || !/^[0-9a-f-]{36}$/i.test(id))
    return data({ ok: false, error: "알림 ID가 올바르지 않습니다." }, { status: 400, headers });
  const { error } = await (client as any)
    ...
    .eq("user_id", user.id)
    .is("read_at", null);
  if (error) return data({ ok: false, error: error.message }, { status: 400, headers });
...
export default function DrawingNotifications({ loaderData, actionData }: Route.ComponentProps) {
  const unread = loaderData.notifications.filter((item) => !item.read_at).length;
  return (
    <main className="mx-auto w-full max-w-3xl px-4 pb-24 pt-8 sm:px-6">
      <Link className="text-sm text-muted-foreground underline underline-offset-4" to="/workspace">← 프로젝트</Link>
      <header className="mt-5 flex items-start justify-between gap-4 border-b pb-5">
        <div>
          <p className="flex items-center gap-2 text-sm font-semibold text-primary"><Bell className="size-4" /> 협업 알림</p>
          <h1 className="mt-2 text-3xl font-bold">알림 작업함</h1>
          <p className="mt-2 text-sm text-muted-foreground">내가 만든 이슈, 내게 배정된 일, 확인 요청을 한곳에서 봅니다.</p>
        </div>
        <span className="rounded-full bg-primary px-3 py-1 text-sm font-bold text-primary-foreground">안 읽음 {unread}</span>
      </header>
      {actionData?.error ? <p className="mt-4 rounded-xl bg-destructive/10 p-3 text-sm text-destructive" role="alert">{actionData.error}</p> : null}
      <div className="mt-6 space-y-3">
        {loaderData.notifications.map((item) => (
          <article className={`rounded-2xl border p-4 ${item.read_at ? "bg-muted/30" : "border-primary/40 bg-primary/5"}`} key={item.id}>
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-muted-foreground">{eventLabels[item.event?.event_type ?? ""] ?? "도면 이슈가 변경되었습니다"}</p>
                <h2 className="mt-1 truncate font-bold">{item.issue?.title ?? "삭제되거나 접근할 수 없는 이슈"}</h2>
              ...
              {!item.read_at ? (
                <Form method="post">
                  ...
                  <Button className="min-h-11" size="sm" type="submit" variant="outline"><Check className="size-4" /> 읽음 처리</Button>
                </Form>
              ) : null}
            </div>
        ...
        {loaderData.notifications.length === 0 ? <p className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">아직 받은 알림이 없습니다.</p> : null}
      </div>
    </main>
  );
}
