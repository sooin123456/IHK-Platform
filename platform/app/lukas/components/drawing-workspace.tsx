...
function drawingShortcutTargetIsEditable(target: EventTarget | null) {
  ...
  return (
    ...
    Boolean(candidate.closest?.("[role='menu']")) ||
...
export default function DrawingWorkspaceClient({
  ...
  const [nativeDwgImportStatus, setNativeDwgImportStatus] = useState<
    string | null
  >(null);
  ...
  const collaborationPhase =
    collaborationConnectionState.lifecycleKey === persistenceLifecycleKey
      ...
      : collaborationEnabled
        ? "connecting"
        : "degraded";
  const connectionSummary = drawingConnectionSummary({
    collaborationEnabled,
    collaborationPhase,
  ...
  const enqueueDrawingMutation = useCallback(
    (
      ...
      if (nativeImportPendingRef.current && trackUndurableWork)
  ...
  const availableDwgSources =
    sourceBundle?.catalog.filter((item) => item.kind === "dwg") ?? [];
  ...
  const importNativeSymbol = async (key: string, clientRequestId: string) => {
    if (
      !nativeImportReady ||
      nativeImportPendingRef.current ||
      commandQueueSizeRef.current !== 0
    )
    ...
    try {
      ...
      for (const [name, value] of Object.entries({
        ...
        key,
        version: "1",
        revisionId: drawingStateRef.current.revisionId,
        clientRequestId,
      }))
        form.set(name, value);
      ...
      if (
        !response.ok ||
        result.ok !== true ||
        result.kind !== "native_symbol_import"
      )
        throw new Error(
          result.error ||
            "기본 심볼을 추가하지 못했습니다. 같은 요청으로 다시 시도하세요.",
        );
  ...
  const nativeDwgPersistenceReady =
    nativeDwgCanRequest &&
    saveState.online &&
    outboxReady &&
    exportCheckpointReady &&
    ...
    !dxfImportPending;
  const nativeDwgImportReady =
    nativeDwgPersistenceReady && localMutationCount === 0;
  const nativeDwgTargetKey = [
    currentUserId,
    revision.project_id,
  ...
  const nativeDwgApplyContextRef = useRef({
    key: nativeDwgTargetKey,
    ready: nativeDwgPersistenceReady,
    sources: availableDwgSources,
  });
  nativeDwgApplyContextRef.current = {
    key: nativeDwgTargetKey,
    ready: nativeDwgPersistenceReady,
    sources: availableDwgSources,
  };
  useEffect(() => {
    setNativeDwgImportStatus(null);
  }, [nativeDwgTargetKey]);
  const applyPreparedNativeDwgImport = async (
    plan: PreparedNativeDrawingDwgProjectImport,
  ) => {
    if (!nativeDwgImportReady || commandQueueSizeRef.current !== 0)
      throw new DrawingNativeDwgImportClientError(
        "현재 작업을 저장하고 체크포인트를 동기화한 뒤 DWG를 가져오세요.",
      );
    ...
    const completed = await enqueueDrawingMutation(
      async (latest) => {
        const assertCurrentTarget = () => {
          ...
          if (
            current.key !== expectedTarget ||
            !current.ready ||
            !authorityCanWriteRef.current ||
            revisionStatusRef.current !== "draft" ||
            !navigator.onLine ||
            undurableMutationCountRef.current !== 1 ||
            latest.revisionId !== revision.id ||
            !drawingState.activeCanvasId ||
            !latest.structure?.canvases[drawingState.activeCanvasId] ||
            !plan.operations.length ||
            !plan.sources.length ||
            plan.operations.some(
              (operation) => operation.revisionId !== latest.revisionId,
            ) ||
            plan.layers.some(
              (layer) => layer.canvasId !== drawingState.activeCanvasId,
            ) ||
            plan.sources.some(
              (source) =>
                !current.sources.some(
                  (file) =>
                    file.id === source.sourceFileId &&
                    file.sha256 === source.sourceSha256,
                ),
            )
          )
            throw new DrawingNativeDwgImportClientError(
              "DWG 대상 캔버스, 원본 또는 편집 권한이 변경되었습니다. 현재 대상에서 다시 확인하세요.",
            );
        };
        ...
        if (!bridge)
          throw new DrawingNativeDwgImportClientError(
            "DWG 편집 연결이 아직 준비되지 않았습니다.",
          );
        const prepared = await applyNativeDrawingDwgImportOperations(
          latest,
          currentUserId,
          plan.operations,
        ...
        assertCurrentTarget();
        if (bridge !== collaborationCommandRef.current)
          throw new DrawingNativeDwgImportClientError(
            "DWG 편집 연결이 변경되었습니다. 다시 확인하세요.",
          );
        nativeDwgImportSendGate.rememberPrepared(
          plan.requestId,
          plan.operations,
        );
      ...
      (cause) => {
        failure = cause;
        if (!(cause instanceof DrawingNativeDwgImportClientError))
          markStorageFailed(cause);
      },
    );
    if (!completed)
      throw (
        failure ??
        new DrawingNativeDwgImportClientError(
          "DWG 가져오기를 시작하지 못했습니다. 현재 편집 상태를 확인하세요.",
        )
      );
  };
  useEffect(() => {
  ...
  return (
    <main className="drawing-workspace flex min-h-screen flex-col bg-slate-100 text-slate-900 xl:h-dvh xl:min-h-0 xl:overflow-hidden xl:[contain:strict]">
      ...
      <header className="drawing-workspace-topbar relative z-[60] flex min-h-16 flex-wrap items-center gap-2 border-b border-slate-200 bg-white px-3 py-2 sm:px-4">
        <Link
          aria-label="도면 목록으로 돌아가기"
          className="inline-flex min-h-10 items-center gap-1 rounded-md px-2 text-sm text-slate-600 hover:bg-slate-100 hover:text-slate-900"
          to={roomUrl}
        >
          <ArrowLeft className="size-4" />
          <span className="drawing-workspace-topbar-label">도면 목록</span>
        </Link>
        {boqReturnHref ? (
          <Link
            className="inline-flex min-h-10 items-center rounded-md px-2 text-sm font-semibold text-indigo-700 hover:bg-slate-100 hover:text-slate-900"
            to={boqReturnHref}
          >
            내역으로 돌아가기
          </Link>
        ) : null}
        <div className="drawing-workspace-topbar-title min-w-0 flex-1 border-l border-slate-200 pl-3">
          ...
          />
          <p className="drawing-workspace-topbar-subtitle truncate text-xs text-slate-500">
        ...
        <div className="drawing-workspace-topbar-actions flex flex-wrap items-center gap-1">
          <span
            ...
            className={`inline-flex min-h-9 items-center gap-1 px-2 text-xs ${saveStatus === "저장됨" ? "text-emerald-700" : saveStatus === "충돌 검토 필요" ? "text-red-700" : "text-amber-700"}`}
            role="status"
          >
            <Cloud className="size-4" />
            {saveStatus}
          </span>
          <details className="relative" data-workspace-connections>
            <summary
              aria-label={`연결 상태: ${connectionSummary.label}`}
              className={`flex min-h-9 cursor-pointer list-none items-center gap-1.5 rounded-lg px-2 text-xs hover:bg-slate-100 focus-visible:outline-2 focus-visible:outline-indigo-500 ${connectionSummary.tone === "healthy" ? "text-emerald-700" : connectionSummary.tone === "warning" ? "text-amber-700" : "text-slate-600"}`}
            >
              <span
                aria-hidden="true"
            ...
            <div className="absolute right-0 top-full z-[70] mt-2 grid w-64 gap-2 rounded-xl border border-slate-200 bg-white p-3 text-slate-700 shadow-lg">
              <span
          ...
          {decisionFields ? (
            ...
            >
              ...
              <input
                className="min-h-9 w-40 rounded-md border border-slate-200 bg-slate-50 px-2 text-sm"
      ...
      {actionError ? (
        <p
          className="border-b border-red-500/30 bg-red-50 px-4 py-2 text-sm text-red-900"
          role="alert"
        >
          {actionError}
        </p>
      ...
      {legacyOperationCount > 0 ? (
        <div
          className="border-b border-amber-500/30 bg-amber-50 px-4 py-2 text-sm text-amber-900"
          role="status"
        >
      ...
      <div className="drawing-workspace-modebar flex flex-wrap items-center gap-2 border-b border-slate-200 bg-white px-4 py-2">
        ...
        <div
          role="radiogroup"
          aria-label="작업 모드"
          className="flex rounded-lg border border-slate-200 bg-slate-50/60 p-1 shadow-inner"
        ...
        >
          {drawingWorkspaceModes.map((mode) => (
            <button
              ...
              className={`min-h-9 rounded-md px-3 text-xs font-semibold ${workspaceMode === mode.id ? "bg-indigo-500 text-white" : "text-slate-600 hover:bg-slate-100"}`}
        ...
        <span aria-hidden="true" className="mx-1 h-6 w-px bg-slate-100" />
        <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">
          보기
        </span>
        <div
        ...
        >
          {(
            ...
            <button
              ...
              className={`min-h-10 rounded-md px-3 text-xs font-semibold ${activeView === mode ? "bg-indigo-500 text-white" : "text-slate-600 hover:bg-slate-100"}`}
        ...
        {sourceBundle?.error ? (
          <p
            aria-label="도면 원본 오류"
            className="rounded-md border border-amber-400/30 bg-amber-50/60 px-3 py-2 text-xs text-amber-900"
            role="alert"
          >
            {sourceBundle.error}
          </p>
        ...
        {sourceBundle && availableDxfSources.length > 0 ? (
          <dxfImportFetcher.Form
            ...
            className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-slate-50/60 p-1"
            method="post"
          ...
          >
            ...
            <label className="flex min-h-10 items-center gap-2 text-xs font-semibold text-slate-600">
              ...
              <select
                ...
                className="min-h-10 max-w-64 rounded-md border border-slate-200 bg-slate-50 px-2 text-sm text-slate-900"
        ...
        {sourceBundle && authorityCanWrite ? (
          <Link
            className="min-h-10 rounded-md border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700"
        ...
        {sourceBundle && drawingState.activeCanvasId ? (
          <DrawingNativeDwgImport
            ...
            sources={availableDwgSources}
            initialSourceId={
              availableDwgSources.some(
                (source) => source.id === requestedDwgSourceId,
              )
                ? requestedDwgSourceId
                : null
            }
        ...
        {sourceBundle && authorityCanWrite ? (
          <Link
            className="min-h-10 rounded-md border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700"
            to={drawingWorkspaceDwgUploadPath(
              revision.project_id,
              workspace.document.id,
            )}
          >
        ...
        {sourceBundle && background.kind === "pdf" ? (
          previousPdfEvidence ? (
            <div
              ...
              className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 p-1 text-xs text-slate-900"
              role="group"
            >
              {(
                [
                ...
                <button
                  ...
                  className={`min-h-9 rounded px-2 font-semibold ${pdfCompareMode === mode ? "bg-amber-400 text-slate-950" : "text-slate-700"}`}
                  key={mode}
              ...
              {pdfCompareMode !== "current" && !exactPreviousPdf ? (
                pdfCompareTransitionPending ? (
                  ...
                  !pdfCompareFetcher.data.ok ? (
                  <span className="text-amber-800" role="alert">
        ...
        {activeView === "split" ? (
          <div
            aria-label="분할 보기 패널"
            className="ml-auto flex rounded-md border border-slate-200 p-1 md:hidden"
            role="tablist"
          >
            {(
              [
              ...
              <button
                ...
                className={`min-h-10 rounded px-3 text-xs font-semibold ${narrowSplitTab === tab ? "bg-indigo-500 text-white" : "text-slate-600"}`}
      ...
      >
        <aside
          aria-label="도면 도구 패널"
          className="drawing-workspace-tools order-2 flex min-h-0 max-h-[32rem] flex-col overflow-hidden border-b border-slate-200 bg-white xl:order-1 xl:max-h-[calc(100vh-3.5rem)] xl:border-b-0 xl:border-r"
          hidden={!leftDockOpen}
        >
          <div
            role="tablist"
            aria-label="도면 도구"
            data-drawing-shortcuts="ignore"
            className="grid shrink-0 grid-cols-4 gap-1 border-b border-slate-200 p-2 xl:grid-cols-2"
          >
            {drawingWorkspacePanels
              ...
              .map((panel) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-9 shrink-0 rounded-md px-2 text-xs font-semibold ${selected ? "bg-indigo-500 text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}
          ...
          >
            ...
            >
              <h2 className="font-bold text-slate-900">댓글·이슈</h2>
              ...
              {revisionRelinkResult ? (
                <p
                  className="rounded-md border border-emerald-400/30 bg-emerald-50/60 p-2 text-xs text-emerald-900"
                  role="status"
                >
              ...
              {revisionRelinkMessage ? (
                <p className="rounded-md bg-slate-50 p-2 text-xs" role="status">
                  {revisionRelinkMessage}
                </p>
              ) : null}
              {visibleRevisionReview.length ? (
                <section
                  ...
                  className="space-y-2 rounded-md border border-amber-300/30 bg-amber-50/30 p-2"
                >
                  <div>
                  ...
                  {visibleRevisionReview.map((candidate) => {
                    ...
                    return (
                      <article
                        className="space-y-2 rounded-md border border-slate-200 bg-slate-50/70 p-2"
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button
                            aria-pressed={active}
                            className="min-h-9 shrink-0 rounded-md border border-amber-300/50 px-2 text-xs font-semibold text-amber-900 disabled:opacity-40"
                        ...
                        {active && revisionRelinkDraft ? (
                          <div className="space-y-2">
                            {candidate.sourceKind === "pdf_region" ? (
                              <>
                                ...
                                >
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    {(
                                      ...
                                      >
                                        <span>{label}</span>
                                        <input
                                          className="min-h-9 w-full rounded-md border border-slate-300 bg-slate-50 px-2 text-xs text-slate-900"
                                          inputMode="decimal"
                                          max={100}
                                          min={0}
                                          onChange={(event) =>
                                  ...
                                  <button
                                    className="min-h-9 w-full rounded-md border border-amber-300/50 px-2 text-xs font-semibold text-amber-900"
                                ...
                                <button
                                  className="min-h-9 w-full rounded-md border border-slate-300 px-2 text-xs font-semibold"
                                  disabled={!pdfPageTransform}
                                  onClick={() => {
                                    setRevisionRelinkDraft((current) =>
                                      current
                            ...
                            >
                              ...
                              <textarea
                                className="min-h-20 w-full rounded-md border border-slate-300 bg-slate-50 p-2 text-xs"
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <button
                                  className="min-h-10 rounded-md border border-slate-300 px-2 text-xs font-semibold"
                                  onClick={() => {
                                    setRevisionRelinkDraft(null);
                  ...
                  {revisionReviewPreviousHref ? (
                    <Link
                      className="block min-h-9 rounded-md border border-amber-300/50 px-2 py-2 text-center text-xs font-semibold text-amber-900"
                      preventScrollReset
              ...
              {canComment ? (
                ...
                >
                  ...
                  <input
                    className="min-h-10 w-full rounded-md border border-slate-300 bg-slate-50 px-2"
                  ...
                  <textarea
                    className="min-h-20 w-full rounded-md border border-slate-300 bg-slate-50 p-2"
                ...
                <>
                  ...
                  transient.selectedIds.find(
                    ...
                    <Form method="post">
                      ...
                      />
                      <button
                        className="min-h-10 w-full rounded-md border border-slate-300 px-3 text-xs font-semibold"
                  ...
                  {canComment ? (
                    <>
                      ...
                      >
                        ...
                        />
                        <div className="col-span-2 rounded-md bg-slate-50 p-2 text-xs">
                          {canvasRegionPickerArmed ? (
                            <p role="status">
                        ...
                        <button
                          className="col-span-2 min-h-10 rounded-md border border-amber-300/60 px-3 text-xs font-semibold text-amber-900 disabled:opacity-50"
                        ...
                        canvasRegionDraftIsCurrent ? (
                          <button
                            className="col-span-2 min-h-10 rounded-md border border-slate-300 px-3 text-xs font-semibold"
                            onClick={() => {
                              setCanvasRegionPickerBoundaryKey(null);
                              setCanvasRegionDraft(null);
                              setCanvasRegionLabel("");
                        ...
                        <label className="col-span-2 text-xs">
                          영역 설명
                          <input
                            className="mt-1 min-h-10 w-full rounded-md border border-slate-300 bg-slate-50 px-2"
                            maxLength={240}
                            name="label"
                            onChange={(event) =>
              ...
              <Link
                className="inline-flex min-h-10 items-center rounded-md border border-slate-300 px-3 font-semibold hover:bg-slate-100"
                to={
                  file
              ...
              <Link
                className="block text-xs text-indigo-600 underline underline-offset-4"
          ...
          >
            <section
              ...
              className="space-y-3 text-sm text-slate-700"
            >
              <h2 className="font-bold text-slate-900">변경 이력</h2>
              <p className="text-xs text-slate-500">
              ...
              {historyStatus ? (
                <p
                  aria-live="polite"
                  className="rounded-md bg-slate-100 p-2 text-xs"
                >
                  {historyStatus}
                </p>
              ) : null}
              ...
              revision.checkpoints.length ? (
                ...
                >
                  ...
                  {revision.checkpoints.map((checkpoint) => (
                    <button
                      className="min-h-10 w-full rounded-md border border-slate-300 px-2 text-left text-xs"
                      disabled={
                        !outboxReady ||
                        !authorityCanWrite ||
                        !canPersistDrawingMutation(
                ...
                {(
                  ...
                  >
                    ...
                    {drawingState.operations.some(
                      ...
                      <button
                        className="mt-2 min-h-9 rounded border border-slate-300 px-2 text-xs font-semibold"
                        onClick={() =>
              ...
              {activityPage?.nextCursor ? (
                <Link
                  className="inline-flex min-h-10 items-center text-xs font-semibold text-indigo-600 underline underline-offset-4"
          ...
          >
            <DrawingBlocksPanel
              nativeCatalogUrl={
                previewMode
                  ? undefined
                  : `/projects/${projectId}/drawing-native-assets?kind=block`
              }
              nativeImportReady={nativeImportReady}
              nativeImportNotice={
                !saveState.online
                  ? "오프라인입니다. 연결 후 기본 심볼을 추가할 수 있습니다."
                  : nativeImportPending
                    ? "심볼을 추가하고 저장된 도면을 불러오는 중…"
                    : !nativeImportReady
                      ? "편집 가능한 도면의 연결과 저장이 완료되면 추가할 수 있습니다."
                      : null
              }
              onNativeImport={
                effectiveCapability === "admin" ||
        ...
        <section
          aria-label="도면 캔버스"
          className="drawing-workspace-canvas relative order-1 min-h-[34rem] min-w-0 bg-slate-950 text-slate-100 xl:order-2 xl:min-h-0 xl:overflow-hidden"
          data-edit-ready={editReady ? "true" : "false"}
          aria-busy={!editReady}
        >
          <div className="absolute left-3 top-3 z-50 flex gap-1 rounded-xl border border-slate-200 bg-white p-1 text-slate-700 shadow-sm">
            <Button
          ...
          >
            <div
              ...
              className={`h-full min-h-0 min-w-0 ${activeView === "3d" ? "hidden" : activeView === "split" && narrowSplitTab === "3d" ? "hidden md:block" : "block"}`}
              hidden={
                activeView === "split" &&
                narrowLayout &&
                narrowSplitTab !== "2d"
            ...
            {selectedIfc && ifcActivated ? (
              <aside
                ...
                className={`max-h-[calc(100vh-4rem)] min-h-0 min-w-0 flex-col overflow-auto border-t border-white/10 bg-background p-4 text-foreground md:border-l md:border-t-0 ${activeView === "2d" ? "hidden" : activeView === "split" ? (narrowSplitTab === "2d" ? "hidden md:flex" : "flex") : "block"}`}
                hidden={
                  activeView === "split" &&
                  narrowLayout &&
                  narrowSplitTab !== "3d"
              ...
              >
                ...
                <p className="mt-2 shrink-0 text-sm leading-6 text-slate-400">
                  GlobalId 연결을 기준으로 2D 객체와 IFC 요소를 함께 찾습니다.
                </p>
                {ifcMatch.status === "ambiguous" ? (
                  <div className="mt-3 shrink-0 rounded-md border p-3 text-sm">
                ...
                {ifcMatch.status === "no_match" ? (
                  <div className="mt-3 shrink-0 rounded-md border border-amber-400/30 bg-amber-50 p-3 text-sm text-slate-900">
                ...
                {ifcLoadError ? (
                  ...
                  <div
                    className={`mt-4 ${activeView === "split" ? "min-h-0 flex-1" : ""}`}
                  >
                    <IfcViewer
          ...
          <nav
            aria-label="캔버스 도구"
            className={`drawing-workspace-toolbar absolute bottom-4 left-1/2 z-50 -translate-x-1/2 flex-nowrap items-center justify-start gap-0.5 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-1.5 text-slate-700 shadow-lg [&_button]:shrink-0 [&_button[aria-pressed=true]]:bg-indigo-50 [&_button[aria-pressed=true]]:text-indigo-700 ${activeView === "3d" || (activeView === "split" && narrowLayout && narrowSplitTab === "3d") ? "hidden" : "flex"}`}
            style={
              activeView === "split" && !narrowLayout
          ...
          >
            <span aria-label="캔버스 작성 도구" className="sr-only">
              작성
            </span>
            <Button
              aria-label="선택 도구"
              ...
              >
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent
                    align="center"
                    aria-label="건축 객체 도구"
                    className="w-44 border-slate-200 bg-white p-1.5 text-slate-700 shadow-lg"
                    side="top"
                    sideOffset={8}
                  >
                    {(
                      ...
                      <DropdownMenuItem
                        aria-label={label}
                        className="min-h-10 px-3 text-sm focus:bg-indigo-50 focus:text-indigo-700"
                        key={tool}
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      ...
                      <span data-tool-label="도형">도형</span>
                  ...
                  <DropdownMenuContent
                    ...
                    sideOffset={8}
                  >
                    <DropdownMenuRadioGroup value={transient.activeTool}>
                    {(
                      [
                        ["polyline", "폴리라인", Waypoints],
                      ...
                      <DropdownMenuRadioItem
                        aria-label={label + " 도구"}
                        className="min-h-10 gap-2 pr-3 focus:bg-indigo-50 focus:text-indigo-700"
                        key={tool}
                        value={tool}
                        onSelect={() => setAuthorizedTool(tool)}
                      >
                        <Icon className="size-4" />
                        <span data-tool-label={label}>{label}</span>
                      </DropdownMenuRadioItem>
                    ))}
                    </DropdownMenuRadioGroup>
                  </DropdownMenuContent>
        ...
        <aside
          aria-label="속성 검사기"
          className="drawing-workspace-inspector order-3 min-h-0 max-h-[28rem] overflow-y-auto border-t border-slate-200 bg-white p-3 xl:max-h-none xl:border-l xl:border-t-0"
          hidden={!inspectorOpen}
        >
          <div
            aria-label="검사기 보기"
            className="mb-3 grid grid-cols-2 gap-1 rounded-lg bg-slate-50 p-1"
            role="tablist"
          >
            <button
              aria-controls="drawing-estimate-result-panel"
              aria-selected={inspectorMode === "result"}
              className="min-h-9 rounded-md px-3 text-sm font-medium text-slate-600 aria-selected:bg-white aria-selected:text-indigo-700 aria-selected:shadow-sm"
          ...
          >
            {visitedInspectorModes.has("object") ? (
              <>
                <h2 className="sr-only">객체 검사기</h2>
                {collaborationEditNotice ? (
                  <p
                    aria-label="공동 편집 작업 차단 안내"
                    className="mb-3 max-w-full break-words rounded-md border border-amber-400/30 bg-amber-50/60 p-2 text-xs leading-5 text-amber-900"
                    role="status"
                  >
                    {collaborationEditNotice}
                  </p>
                ...
                {selectedDrawingObjectId ? (
                  <>
                    ...
                    >
                      <summary className="cursor-pointer text-sm font-semibold text-slate-700">
                        ...
                        <span className="ml-1 text-xs font-normal text-slate-500">
                          {selectedObjectLineage.completed}/
                          {selectedObjectLineage.total} 연결
                        </span>
                      </summary>
                      <div aria-label="업무 계보">
                        <ol
                          aria-label="선택 객체 업무 계보"
                          className="mt-3 grid grid-cols-6 overflow-hidden rounded-md border border-slate-200 bg-slate-50/70 text-center text-[10px] font-semibold"
                        >
                          {(
                            [
                            ...
                            <li
                              className={`border-r border-slate-200 px-1 py-2 last:border-r-0 ${linked ? "bg-emerald-500/15 text-emerald-800" : "text-slate-500"}`}
                    ...
                    <section
                      ...
                      className="mb-3 rounded-lg border border-slate-200 bg-slate-50/60 p-3 text-xs"
