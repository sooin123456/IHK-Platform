...
export const projectSourceUploadDirectory = "source-uploads";
...
export function isProjectStorageObjectPath({
  ownerId,
  projectId,
  originalFilename,
  storagePath,
  directory,
}: Omit<StorageObjectPathOptions, "objectId"> & {
  storagePath: string;
}) {
  const segments = storagePath.split("/");
  const objectName = segments[directory ? 3 : 2] ?? "";
  if (segments.length !== (directory ? 4 : 3) || objectName.length < 36)
    return false;
  try {
    return (
      storageObjectPath({
        ...
        projectId,
        originalFilename,
        objectId: objectName.slice(0, 36),
      }) === storagePath
    );
  } catch {
    return false;
