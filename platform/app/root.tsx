...
export async function loader({ request }: Route.LoaderArgs) {
  // Only browser-safe Supabase settings are required for the current workspace.
  // The service-role key is intentionally not needed until a server-only
  // signed-download/share endpoint is added.
  if (
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ThemeProvider
      specifiedTheme={data?.theme ?? "light"} // Operational screens default to the clearest theme.
...
function InnerLayout({ children }: { children: React.ReactNode }) {
  ...
  const isPreRendered =
    pathname.includes("/legal") || pathname.includes("/news");
